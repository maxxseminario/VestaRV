// VestaRV: rv4th Forth-like interpreter, boot ROM build
...
// The seven arrays declared further down are the whole of the interpreter's mutable
// state and all live in .noinit, which MCU-bootrom.ld places at the top of the private
// TCM. The TCM is 8 KiB (MemoryMap RamSize), its first 484 bytes are the interrupt
// vector table and the top 1 KiB is the C stack, so everything here must fit the 6684
// bytes between them; the sizes below use 5696 plus 48 bytes of scalars.
// An overrun is silent without a link check: .noinit is NOBITS, so it never reaches the
// ROM image, and past 0x9FFF it lands in the TCM's own upper mirror, overwriting the
// vector table, these scalars and the stack. The ASSERT at the bottom of MCU-bootrom.ld
// fails the link if the total stops fitting.
