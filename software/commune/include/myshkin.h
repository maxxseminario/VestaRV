...
// C view: peripheral base addresses, register address constants and the bare memory-mapped register macros.
...
// The bare SYSTEM register macros and the generated structs in software/include/regs/
// are MUTUALLY EXCLUSIVE per translation unit: SYSCLKCR, CLKDIVCR, CRCDATA, CRCSTATE,
// WDTPASS, WDTCR, WDTSR and WDTVAL are object-like macros here and struct members
// there, so leaving both live makes system_t fail to parse. The switch below picks
// one: default is the bare macros; -DVESTA_REGS_STRUCTS, or including a regs header
// first, suppresses them. The <REG>_ADDRESS and <REG>_PTR(base) forms stay in both.
...
// PERIPH_SARADC0_BASE is deliberately absent: 0x4B00 is PWRCTRL on Castalia, so the
// name aimed a SARADC write at PWRCR. PERIPH_AFE0_BASE names the rev-1 afe_stub bank
// and retires with it.
