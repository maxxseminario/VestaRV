...
{
    MMR_32_BIT_MACRO(SYSCLKCR_ADDRESS) = 0;              /* trap (a): SMCLK -> HFXT, FIRST (TRM 15.1) */
    MMR_32_BIT_MACRO(T0CMP0) = PWM_DUTY;                 /* duty compare   (TRM 14) */
    MMR_32_BIT_MACRO(T0CMP2) = PWM_PERIOD;               /* period compare (TRM 14) */
    MMR_32_BIT_MACRO(T0CR)   = TEN_BIT | CMP2RST_BIT | SSEL_SMCLK | DIV_4;
}
...
static int timer0_wait_counting(void)
{
    ...
    for (budget = POLL_BUDGET; budget; budget--) {
        if (MMR_32_BIT_MACRO(T0VAL) != first)            /* a FRESH read advanced */
            return (POLL_BUDGET - budget) + 1;
    }
    return 0;                                            /* never started counting */
}
