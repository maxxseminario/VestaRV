 ...
 * a byte-transmit that follows the correct status-flag discipline, then a
 * self-check transmits a known string and verifies the TX-complete flag (TCIF)
 * sequences 0 -> 1 -> 0 for every byte, all with bounded polls. The pass gate is
 * pass()/fail(); nothing here relies on an external wire -- TCIF is asserted by
 * the UART core itself when the frame finishes.
 */
#include "course.h"
...
/* ======================================================================= */
/* PART 1 -- the UART1 driver.  (Reference: implemented.)                   */
/* ======================================================================= */
...
{
    MMR_32_BIT_MACRO(SYSCLKCR_ADDRESS) = 0;          /* SMCLK -> HFXT (TRM 15.1) */
    MMR_32_BIT_MACRO(U1BR)  = U1_BAUD_DIV;           /* baud divisor  (TRM 13.4) */
    MMR_32_BIT_MACRO(U1CR)  = UEN_BIT;               /* enable, no interrupts    */
}
...
{
    int budget;
    (void)MMR_32_BIT_MACRO(U1SR);                    /* 1: read + discard */
    MMR_32_BIT_MACRO(U1TX) = (uint8_t)c;             /* 2: start the frame */

    for (budget = TX_BUDGET;                          /* 3: wait complete */
         budget && !(MMR_32_BIT_MACRO(U1SR) & TCIF_BIT);
         budget--) { }
    if (budget == 0) return 0;                        /* TCIF never set */

    MMR_32_BIT_MACRO(U1SR) = TCIF_BIT;               /* 4: clear the pulse */

    { int b2;                                         /* 5: confirm cleared */
      for (b2 = TX_BUDGET;
           b2 && (MMR_32_BIT_MACRO(U1SR) & TCIF_BIT);
           b2--) { }
      if (b2 == 0) return 0; }

    return (TX_BUDGET - budget) + 1;                  /* polls consumed, >= 1 */
}
