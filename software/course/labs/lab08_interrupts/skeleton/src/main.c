/* lab08_interrupts -- STUDENT STARTING POINT.
 *
 ...
 *
 * As shipped this BUILDS but FAILS: all four functions are empty, so no
 * interrupt is ever armed and both ISR counts stay 0. main() calls fail()
 * (a0 = 0xDEADBEEF) after the bounded waits. The console still prints the two
 * "ISR entries = 0" lines (the negative control). Implement the four functions
 * to make it PASS with nonzero counts on both paths.
 *
 * NOTE: implement the ISRs' level-clear FIRST -- an armed interrupt whose
 * handler forgets to clear its level re-fires forever (a hang, not a fail).
 */
#include "course.h"
#include "irq.h"
...
/* ======================================================================= */
/* PART 1 -- the four functions you implement.  IMPLEMENT THESE FOUR.         */
/* ======================================================================= */
...
static void arm_timer_tick(void)
{
     ...
     *   MMR_32_BIT_MACRO(CLINT_MTIMECMP_LO(0)) = now + MTIP_DELTA;   -- lo first
     *   MMR_32_BIT_MACRO(CLINT_MTIMECMP_HI(0)) = 0;                  -- then arm
     */
}
...
static void timer_isr(void)
{
     ...
     *       uint32_t now = MMR_32_BIT_MACRO(CLINT_MTIME_LO);
     *       MMR_32_BIT_MACRO(CLINT_MTIMECMP_LO(0)) = now + MTIP_DELTA;  -- re-arm
     *   } else {
     *       MMR_32_BIT_MACRO(CLINT_MTIMECMP_HI(0)) = 0xFFFFFFFFu;       -- disable
     *   }
     */
}
...
{
    /* TODO(lab08):
     *   MMR_32_BIT_MACRO(SYSCLKCR_ADDRESS) = 0;                       -- clock first
     *   MMR_32_BIT_MACRO(IRQR_ROW_ENL(0)) |= (1u << IRQ_SRC_TIM0_CMP0); -- route 19
...
{
    /* TODO(lab08):
     *   if (id != IRQ_SRC_TIM0_CMP0) return;
     *   ext_count++;
     *   MMR_32_BIT_MACRO(T0SR) = CMP0IF_BIT;          -- clear the LEVEL (write-1)
     *   if (ext_count >= EXT_TARGET) MMR_32_BIT_MACRO(T0CR) = 0;   -- stop
     */
    (void)id;
}
