/* lab04_gpio -- STUDENT STARTING POINT.
 *
 * GPIO at the register level (TRM ch. 11). You configure pin directions, drive
 ...
 * PxSEL / PxAFS. Every check reads back what you wrote through the peripheral's
 * OWN registers -- no external pin loopback. Progress prints go to the UART0
 * console via the SDK (console_init / printf); the pass gate is pass()/fail().
 *
 * As shipped this BUILDS but FAILS: the three PART 1 functions are empty, so the
 * direction / output / pinmux readbacks all mismatch and main() calls fail()
 * (a0 = 0xDEADBEEF). Implement the three functions to make it PASS.
 *
 * Port under test: GPIO1 (0x4100). Pin under test for the pinmux flip: P1.0,
 * whose alternate function 1 (AF1) is the TIMER0 compare output T0CMP0 (TRM 11.3).
 */
#include "course.h"
...
{
    /* TODO(lab04): write set_mask to PxOUTS, then clr_mask to PxOUTC.
     *   GPIO_W(base, PxOUTS_OFF, set_mask);
     *   GPIO_W(base, PxOUTC_OFF, clr_mask);
     */
    (void)base; (void)set_mask; (void)clr_mask;
}
...
{
    /* TODO(lab04):
     *   1. Read PxAFS, replace pin's 4-bit field with `af`, write it back:
     *        afs = GPIO_R(base, PxAFS_OFF);
     *        afs = (afs & ~AFS_FIELD_MASK(pin)) | AFS_FIELD(pin, af);
     *        GPIO_W(base, PxAFS_OFF, afs);
     *   2. Set the pin's PxSEL bit (read-modify-write):
     *        GPIO_W(base, PxSEL_OFF, GPIO_R(base, PxSEL_OFF) | (1u << pin));
     */
    (void)base; (void)pin; (void)af;
}

/* Return a pin to GPIO (primary) mode: clear its PxSEL bit. (Provided.) */
