...
#
################################################################################
# THE DEBUG-ON STAGED INPUTS, and why they are not regenerated on a whim.
# (Added 2026-08-07 at the D4 commit wave, ordered by R-D4-2(6).)
#
# This flow needs a debug-ON MCU.vhd + MemoryMap.vhd, which the DEFAULT
# `make chip` does not emit.  Until D4 it read them from ANOTHER SESSION'S
# /tmp SCRATCHPAD -- so the debug-ON assembly pin was one `/tmp` reap away
# from being unreproducible.  They now live beside every other shared
# generated input:
#     $INPUT_DIR/MCU_top_dbgon.gen.vhd        (md5 73c09532556f7f5c653cb0d7bd7bdf75)
#     $INPUT_DIR/MemoryMap_dbgon.gen.vhd      (md5 29326a681f3a681b5293c2b09770bada)
# copied BYTE-IDENTICAL from that scratchpad, so the flow reads exactly the
# bytes the standing pin was measured against.  Nothing about the synthesis
# changed; only where two files are read from.
#
# THE REGEN RECIPE, and it is NOT `config/castalia_debug.json`.
# This build is the SHIPPED DEFAULT PLUS `debug.enable` AND NOTHING ELSE --
# a deliberate isolation ratified by R-D2-9(3): the scored number must not
# carry castalia_debug.json's `priv.umode` term, because the whole point of
# the pin is to price the DEBUG knob, and a prediction for a knob is a
# prediction for everything the knob turns on.  The scratch config never
# enters config/.
#
#     cd ~/vestarv/platform/common
#     printf '{ "debug": { "enable": true } }\n' > /tmp/dbgon_asm.json
#     make chip CONFIG=/tmp/dbgon_asm.json
#     cp out/hdl/MCU.vhd       ../../genus/common/in/MCU_top_dbgon.gen.vhd
#     cp out/hdl/MemoryMap.vhd ../../genus/common/in/MemoryMap_dbgon.gen.vhd
#     make chip                        # RESTORE THE DEFAULT -- mandatory
#     /usr/bin/python3.6 python/check_mcu_vhd.py    # must say STRICT/IDENTICAL
#
# ACCEPTANCE TEST for a regeneration, so the next reader can CHECK rather
# than trust (measured on the files staged here, 2026-08-07):
#     CORE_ENABLE_DEBUG   : boolean := true     <- the knob under test
#     CORE_ENABLE_TRAPCSR : boolean := true     <- shipped default since R-DK3
#     CORE_ENABLE_UMODE   : boolean := false    <- shipped default; NOT castalia_debug's true
# A regenerated MemoryMap with UMODE = true means castalia_debug.json was
# used by mistake; it turns U-mode on in all four tiles and WILL move the
# `sequential` pin (18,163 at the D4 close).  That is the wrong number, not
# a refreshed one.
#
# (D4 note, recorded because the misreading cost a commit-wave stop: the
# UMODE = false above is ISOLATION, not rot.  The 17,970 -> 18,159 -> 18,163
# lineage all measure this same deliberately-isolated build.  What WAS
# defective, and is fixed here, is that the inputs had no provenance
# statement and lived in a foreign /tmp scratchpad, so a reader could not
# tell the two apart.)
################################################################################
...
set TOP_MODULE       MCU
set BASENAME         MCU_MP.genus.dbgon
...
set I2CSCL_FREQ		5
# D3: the JTAG TCK domain (d3_cdc_spec 5). 10 MHz is the conventional JTAG
# target and is INDEPENDENT of the dtmcs.idle protocol assumption in
# jtag_dtm.vhd (TCK <= 7.14 MHz for a zero-busy dwell of 7): a debugger clocking
# faster than that simply sees sticky busy and re-issues, which is a supported
# outcome, while the TAP's own logic must still close at the pin rate.
set TCK_FREQ		10
...
read_hdl -vhdl -library work $INPUT_DIR/MemoryMap_dbgon.gen.vhd
...
read_hdl -vhdl -library work $INPUT_DIR/MCU_top_dbgon.gen.vhd
...
# --- D3 JTAG TCK domain (d3_cdc_spec 5) ---
# TCK is an INDEPENDENT, ASYNCHRONOUS clock. The DTM crosses into mclk on two
# toggle handshakes with the payload held (the UART flags_cdc_proc idiom), so no
# path between the two domains is ever meant to be timed. The mechanism that
# produces that isolation -- and SURVIVES write_sdc, which regenerates every
# exception from the -domain attribute -- is a create_clock carrying its OWN
# -domain. There is no set_clock_groups anywhere in this tree, and a hand-added
# one here would be destroyed by the next write_sdc.
#
# DECLARED ON THE dtm0 INSTANCE PIN, NEVER ON THE TOP-LEVEL PORT. The chip SDC
# generator (innovus/common/MCU_castalia/gen_MCU_castalia_sdc.sh) deletes every
# [get_ports ...] line and FATALs if any survives the transform, so a
# port-declared TCK clock would abort the whole chip flow.
#
# GUARDED, and the guard is not defensive clutter: dtm0 exists ONLY in a
# debug.enable build, and this flow's default configuration has no DTM at all --
# create_clock on a nonexistent hpin is an error that would break the standing
# knob-OFF genus gate. BOTH ARMS ANNOUNCE THEMSELVES, because a silently skipped
# clock is exactly the failure a guard invites: read the log, do not assume.
if {[llength [get_db hinsts *dtm0*]] > 0} {
	puts "D3: dtm0 present -- declaring the clk_tck domain (period $TCK_PERIOD ns)"
	create_clock -name clk_tck	-domain clk_tck_domain		-period $TCK_PERIOD	hpin:$TOP_MODULE/dtm0/tck
	define_cost_group -name clk_tck_group		-weight 1
	path_group -from clk_tck		-group clk_tck_group
} else {
	puts "D3: no dtm0 hinst in this netlist (debug.enable off) -- no clk_tck domain declared"
}
