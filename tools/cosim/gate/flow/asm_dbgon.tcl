# VestaRV: Genus synthesis of the flat four-hart MCU_MP with debug enabled.
# Reads the staged debug-ON MCU.vhd and MemoryMap.vhd named below rather than the
# default `make chip` output. The top entity is MCU and all four harts are
# hart_tile instances hart0..hart3. Time optimised and area relaxed: high effort
# at every stage with no max_area. This flow produces the standing assembly pin,
# so the staged inputs must be regenerated by the recipe below and nothing else.
# The staged debug-ON inputs. `make chip` does not emit them by default, so they
# live beside every other shared generated input:
...
#
# The build is the shipped default plus debug.enable and nothing else. That
# isolation is the point: the pin prices the debug knob alone, so it must not
# carry config/castalia_debug.json's priv.umode term. The scratch config never
# enters config/.
#
...
#
# Acceptance test for a regeneration, so the next reader can check rather than
# trust:
#     CORE_ENABLE_DEBUG   : boolean := true     <- the knob under test
#     CORE_ENABLE_TRAPCSR : boolean := true     <- shipped default
#     CORE_ENABLE_UMODE   : boolean := false    <- shipped default, NOT castalia_debug's true
# UMODE = true in a regenerated MemoryMap means castalia_debug.json was used by
# mistake. It turns U-mode on in all four tiles and moves the sequential pin,
# which is a different number, not a refreshed one.
