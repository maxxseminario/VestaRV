#!/usr/bin/python3.6
# coding: utf-8
"""VestaRV: standalone self-tests for tools/cosim/compare.py. Exit 0 all pass, 1 otherwise.

Every comparator exit code 0 to 5 is exercised, with no external dependency. RTL20 and SPIKE20
are verbatim first-20-line windows of a real tracer output and a real Spike commit log, neither
produced by any code under tools/cosim. The shapes that window lacks are hand-derived from the
wire format, and each such fixture carries a negative control so exit 0 is never vacuous.
"""
...
def main():
    ...
    # counter that has only ever been seen at 0 has not been tested:
...
""".splitlines(True)
    ...
    # disabled amendment, which is the fail-safe direction.
    ...
    # comparator and proved nothing: a detector never seen to
...
""".splitlines(True)
    ...
    # (0x0045a00f -- measured; note it does NOT end `200f`).
...
""".splitlines(True)
    ...
    #     c1957_tcontrol -- and the RTL emits ONE, its
...
""".splitlines(True)
    ...
    #
    # MEASURED SHAPES, taken on the reference
...
""".splitlines(True)
    ...
    # would be an instrument keyed on the
...
""".splitlines(True)
    ...
    # into ascending address order, independently, which is
    # a set compare, obtained without giving up a compared field.  The
...
""".splitlines(True)
    ...
    # end-to-end fixture can show: a case that cannot fail is not
    # evidence).
