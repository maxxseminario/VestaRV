...
| `--amend NAME[,…]` | K2b CONFIG-GATED comparator amendment(s), repeatable. Each reconciles a MEASURED record-shape difference between a knobs-on VestaRV config and the reference, and is enabled only by the knob that owns it — the set is DERIVED from the resolved config by `oracle_isa.py::derive_amendments()` and the default Castalia config derives NONE. An unknown name is a usage error (exit 5), never a silently-disabled amendment. Every application is counted, printed per identity in the stderr summary, and a zero count is printed as **VACUOUS**. Implemented: `zfinx-fflags` (isa.zfinx), `cboz-stores` (isa.zicboz), `cmjt-load` (isa.zcmt). `amend.py`'s header is the authority on what each one does and where a NEW rule belongs. |
