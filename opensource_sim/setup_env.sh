#!/usr/bin/env bash
# VestaRV: set up a fully open-source simulate and verify toolchain: GHDL, a
# Python venv with cocotb, and the xPack RISC-V GCC toolchain.
#   ./opensource_sim/setup_env.sh          install whatever is missing
#   ./opensource_sim/setup_env.sh --check  probe only, nonzero if anything is
#                                          required and missing
# Idempotent: a re-run only fills in what is still absent. After a successful run,
# `source opensource_sim/env.sh`.
set -euo pipefail

# One pinned xPack RISC-V GCC release. CI and the docs assume this version, so
...
# 1. System packages: ghdl, gcc, make, the python3-venv equivalent, curl or wget,
# and xz. apt on Debian trixie or Ubuntu 24.04 and later is the tested path,
# because it ships GHDL 5 or newer. Other managers are best effort: ghdl may be
# absent or too old there, which the version check below catches.
...
# 3. The RISC-V toolchain: use the one on PATH if there is one, otherwise fetch
# the pinned xPack prebuilt release into opensource_sim/tools/, never system wide.
RISCV_BIN_DIR=""
