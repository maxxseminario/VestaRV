"""Analysis orders for the SINGLE-HART chip, config/asic_default.json.
...
//platform/common:chip_artifacts_asic_default. So the spine has to be spliced
...
ASIC_DEFAULT_HEAD and ASIC_DEFAULT_MID are the two runs of tracked files
either side of them. The BUILD file interleaves:
...
is dropped because of the configuration and not for convenience: asic_default
sets peripherals.npu false, so its MemoryMap.vhd declares no MmrAddrNPU* constants
...
ASIC_DEFAULT_MACROS = "hdl/common/macros/macros.vhd"
...
def _split():
    if VESTA_MCU_RTL[1] != _MEMORY_MAP:
        fail("opensource_sim/asic_default/defs.bzl: VESTA_MCU_RTL[1] is %s, not %s. " %
    ...
    if VESTA_MCU_RTL[-1] != _MCU:
        fail("opensource_sim/asic_default/defs.bzl: VESTA_MCU_RTL no longer ends at %s, " % _MCU +
             "so the generated MCU top can no longer simply replace the last " +
             "entry; re-anchor this file.")
    if _NPU not in VESTA_MCU_RTL:
        fail("opensource_sim/asic_default/defs.bzl: %s is no longer in VESTA_MCU_RTL, " % _NPU +
             "so this list's reason for dropping it is stale. Delete the drop.")
    mid = [f for f in VESTA_MCU_RTL[2:-1] if f != _NPU]
    return [ASIC_DEFAULT_MACROS] + VESTA_MCU_RTL[:1], mid

ASIC_DEFAULT_HEAD, ASIC_DEFAULT_MID = _split()
