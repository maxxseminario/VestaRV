...
makefile puts -lgcc before the objects and the course SDK puts it after.
...
COURSE_SDK = "//software/course/sdk"

def course_lab(name, srcs = ["src/main.c"], defines = [], extra_srcs = [], visibility = None):
    """One course lab image, following software/course/sdk/course.mk.

    Every lab makefile is three lines: TARGET, SRC_SOURCES, include course.mk.
    So is every call to this macro.

    Link order matches ALL_OBJS in course.mk: crt0.o first, then the student
    sources, then course.o. -lgcc goes after the objects so its members
    resolve against them.

    -O2 is REQUIRED here and is not an optimization preference. It lowers
    constant integer division to a magic multiply, so the compiler never emits
    the hardware divu/remu instructions, which have a result hazard bug on this
    chip (SDK README, "Known issues"). Student code that divides by a runtime
    value would still emit divu, so keep divisors constant. Do not relax this
    to -Os.

    --gc-sections with -ffunction-sections/-fdata-sections drops the unused
    floating point console helpers in course.c, so no soft float libgcc
    references survive into the image.

    software/course/sdk/generated/ is a frozen snapshot of the chip generator
    output and is deliberately NOT on the include path: chip.h is
    self-contained, nothing in the SDK or the labs includes MemoryMap.h. It is
    kept as reference material and must never be regenerated by a build.

    The labs have no include/ directory, so course.mk's -I$(INC_DIR) matches
    nothing and is left out here.

    Args:
      name: the makefile TARGET, and the image stem.
      srcs: student sources, lab relative, in link order.
      defines: preprocessor defines, without the -D.
      extra_srcs: files the sources reach for but do not compile on their own,
        for example a sibling lab's main.c pulled in by #include.
      visibility: forwarded to every emitted target.
    """
    rv32_firmware(
        name = name,
        srcs = [COURSE_SDK + ":crt0.S"] + srcs + [COURSE_SDK + ":course_lib/course.c"],
        march = "rv32ima",
        mabi = "ilp32",
        defines = defines,
        copts = [
        ...
        hdrs = [COURSE_SDK + ":course_lib_headers"] + extra_srcs,
        includes = ["software/course/sdk/course_lib"],
        ld_script = COURSE_SDK + ":course.ld",
        link_flags = [
            "{march}",
            "{mabi}",
            "-nostartfiles",
        ...
        link_flags_post = ["-lgcc"],
        # Same 0x14000 byte window the blinky family images use.
        pad_bytes = 0x14000,
        visibility = visibility,
    )
