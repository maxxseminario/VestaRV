"""VestaRV: GHDL rules for the open-source simulation tier.

vhdl_source_set picks an ordered subset of a filegroup, ghdl_library analyzes one into a work
library, ghdl_test analyzes and elaborates. The list is curated and ordered because the tree
holds conflicting regfile and ClkGate entities and analysis order matters. A work library is
not relocatable (mcode re-reads sources at run time), so ghdl_test analyzes for itself.
"""
...
def _lib_root(lib_files, std, runfiles):
    """The directory to hand GHDL's -P flag, which wants the one containing std/vXX and ieee/vXX, so
    the compiled std library index is found and that tail stripped. Build actions resolve against
    the exec root and a test binary against its runfiles root.
    """
