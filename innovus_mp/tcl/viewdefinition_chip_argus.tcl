# MMMC view definition -- chip_top_argus CONNECTED chip (A5, M15 Flavor B).
# Same corners/libs as the A4 assembly; the SDC is the mcu0/-prefixed chip
# transform (in/chip_top_argus.sdc -- port load/driver lines dropped, real
# pads are the loads now). tphn pad cells enter LEF-only (timing-less, like
# the analog abstracts -- same IMPVL-366 rule: no dummy libs).
...
create_library_set \
    ...
    -timing [list \
        ...
        "out/hart_tile_argus.etm_ss.lib" \
		]
create_library_set \
...
create_library_set \
    ...
    -timing [list \
        ...
        "out/hart_tile_argus.etm_ff.lib" \
		]
...
create_constraint_mode \
    ...
    -sdc_files "in/chip_top_argus.sdc"
