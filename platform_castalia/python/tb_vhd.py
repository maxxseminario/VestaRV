...
# hdl/MCU_MP/tb/riscv_tb.vhd with the numHarts-dependent regions carved out
...
class TbVhdEmitter():
    ...
    def emitComponentPorts(self):
        n = self.n
        lines = [' ' * 12 + '-- M3b: per-hart pass/fail observation (a0 of the '
    ...
    def emitA0Signals(self):
        ...
        return [
            ' ' * 4 + '-- M3b: a0 of the ' + str(n - 1)
                + ' private-memory harts (boot from preloaded RAM at 0x8200)',
    ...
    def emitPassFailSignals(self):
        ...
        return [
            ' ' * 4 + '-- M3b: latched pass/fail for private-memory harts 1-' + str(n - 1),
    ...
    def emitPortMap(self):
        n = self.n
        lines = [' ' * 8 + '-- M3b: private-memory harts 1-' + str(n - 1)]
        for h in range(1, n):
            term = ',' if h != n - 1 else ''
