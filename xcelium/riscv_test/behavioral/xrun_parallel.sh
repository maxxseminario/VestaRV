...
TEST_FILES=(
    ...
    # ---------------------------------------------------------------------------
    # DISABLED: rv32ua — atomics (A extension, not implemented in the core). Each
    # test traps on an unimplemented instruction and spins (never writes
    # CAFEBABE/DEADBEEF to a0), so it can only end via the watchdog timeout — a
    # slow, uninformative FAIL. Re-enable once the A extension is added.
    # ---------------------------------------------------------------------------
