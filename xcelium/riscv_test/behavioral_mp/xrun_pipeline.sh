#!/bin/bash
# PIPELINED rv32ui test runner — variant of xrun_parallel.sh (this dir):
...
#   3. Elaborate + simulate PIPELINED — one MAX_PARALLEL worker pool where each
#      worker elaborates ONE snapshot then immediately simulates it (later tests
#      elaborate while earlier ones simulate) instead of "elaborate all, THEN
#      simulate all". NOTE: behavioral elaboration has NO SDF and is already
#      fast, so the win here is modest (unlike genus_mp, where SDF elaboration
#      dominates); this exists mainly to give the same option across mp flows.
#
# Usage:
#   ./xrun_pipeline.sh
#   MAX_PARALLEL=8 ./xrun_pipeline.sh
#
...
# Each worker elaborates ONE snapshot then immediately simulates it, and up to
# MAX_PARALLEL workers run at once. Every test elaborates into its OWN snapshot
# (work.tb_xxx:behavioral), so the xmelab calls are independent and overlap the
# in-flight sims of earlier tests. (Behavioral elab is fast/no-SDF, so this
# mostly matters for consistency with genus_mp/xrun_pipeline.sh.)
echo ""
echo "=== [3/3] Pipelined elaborate+simulate (MAX_PARALLEL=$MAX_PARALLEL, ${#ENTITIES[@]} tests) ==="
TOTAL=${#ENTITIES[@]}
# Own status dir (NOT xrun_parallel.sh's .status) so the two scripts' progress
# counters never collide if both run / leftovers linger.
STATUS_DIR="$LOG_PATH/.status_pipe"
rm -rf "$STATUS_DIR"; mkdir -p "$STATUS_DIR"

# Elaborate then simulate one snapshot, then immediately classify and report its
# result, so PASS/FAIL lines stream to the terminal as each test finishes (in
# completion order, not submission order). Per-entity elab log (concurrent elabs
# must not share one file). An elab failure marks the test ELABFAIL (points at
# its elab log) and does NOT abort the whole run.
run_one() {
    local entity="$1"
    # --- elaborate ---
    xmelab -ACCESS +r -licqueue "work.${entity}:behavioral" \
