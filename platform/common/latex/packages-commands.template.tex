...
% Extra packages
% [noforwardlinks]: the list entry otherwise emits \hyperref[acro:XXX] to the
% acronym's FIRST \ac{} use in the body. Nothing in this manual calls \ac{},
% so all 39 of those forward links were dead and every build printed "There
% were undefined references" (D-11, 2026-09-05). The option drops only the
% forward link; the \hypertarget on each list entry stays, so a future \ac{}
% still jumps into the glossary.
