% VestaRV: TRM preamble, copied verbatim to latex/TRM/packages-commands.tex.
% Holds the package list, the figure theme every generated diagram draws from,
% and the address-column and callout macros the template and the emitters use.
...
\usepackage[figuresleft]{rotating}  % Rotate figures; a sideways figure has its top at the right page edge
...
% Extra packages
% [noforwardlinks]: without it each list entry emits \hyperref[acro:XXX] to the
% acronym's first \ac{} use. Nothing in this manual calls \ac{}, so those links are
% dead and the build reports undefined references. The option drops only the forward
% link; the \hypertarget on each entry stays, so a future \ac{} still jumps here.
\usepackage[noforwardlinks]{acronym} % Glossary of terms and abbreviations (chapter 1)
...
% Figure theme: every generated block diagram draws from this one palette and style
% set. Red package boundary and accents, greyscale blocks, sans-serif labels, as the
% whole-chip figure is drawn. vestaRed is exactly red!75!black and vestaRedText exactly
% red!70!black. Emitters use a v-prefixed style below rather than spelling a fill or a
% line width inline; a one-off is written as a modification of one of these.
...
% Cross-references and register names are clickable, in the theme's text red. Set here
% rather than on the \usepackage line because the colour is defined after hyperref.
\hypersetup{hidelinks}

% Running head needs 14pt with the Libertine face; fancyhdr warns on every page otherwise.
\setlength{\headheight}{14pt}

% Callouts, two tiers only: Note (advisory) and Caution (the hardware misbehaves if
% the text is ignored). Both are run-in labelled paragraphs, no box and no colour.
...
% Heading-level shift for an included file that still uses article levels. Use inside
% a group: {\ShiftHeadingsUp \input{file}}. The per-implementation analog chapter is
% the one consumer.
