% I3C chapter. Section order is the per-peripheral template enforced by
...
Because the serial core runs in the SMCLK domain, software must set \register{\register{SYSCLKCR}} to 0 (SMCLK from HFXT) before using the controller, exactly as for the SPI, UART, and I2C peripherals.

In configurations that include it, \peripheral{I3C0} lives in the shared peripheral window behind the multi-core arbiter (see Section \ref{s:multicore}) in the mutex-bank page, sub-slot 1 at \texttt{0x6100}, so any hart can use it.
