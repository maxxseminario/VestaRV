...
\begin{itemize}
  \item If \bitfield{WDTIE} is set \textbf{and} the watchdog vector (vector 0) is routed to some hart in the \peripheral{IRQROUTER}, the watchdog interrupt is delivered through the claim/complete mechanism and the servicing hart executes its handler. If \bitfield{WDTHWRST} is also set, the system resets after that hart completes the claim.
  \item If \bitfield{WDTIE} is clear, or vector 0 is not routed to any hart, the system resets immediately on timeout provided \bitfield{WDTHWRST} is set.
\end{itemize}
