% QSPI chapter. Section order is the per-peripheral template enforced by
...
A transaction is fully described by the control, address, and command registers. The byte-lane-0 write of \register{QSPIxCMD} \emph{launches} it; it is ignored while the controller is disabled or busy. Writing \register{QSPIxTX} never launches a transaction. For a read, the captured word appears in \register{QSPIxRX} and \register{QSPIRXFULL} is set; reading \register{QSPIxRX} has no side effects.

\section{Interrupts and events}

The three status flags \register{QSPITXEIF}, \register{QSPIRXFULL}, and \register{QSPITCIF} are write-1-to-clear: write a 1 to a bit to clear it.

The two interrupt sources are vector 55 (transfer complete) and vector 56 (receive-register full), delivered through the interrupt router like every other shared-peripheral source.
...
In configurations that include it, \peripheral{QSPI0} lives in the shared peripheral window behind the multi-core arbiter (see Section \ref{s:multicore}) at slot \texttt{0x4C00}, so any hart can use it.
