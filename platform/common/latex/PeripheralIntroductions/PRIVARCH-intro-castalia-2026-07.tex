...
\section{Privileged Architecture} \label{s:privarch}
...
\subsection{The Legacy Trap Mechanism} \label{ss:priv-legacy}
...
\subsection{Selecting a Delivery Mode: \register{mtrapctl}} \label{ss:priv-mtrapctl}
...
\subsection{The Machine Trap Registers} \label{ss:priv-csrs}
...
\subsection{Trap Entry, Trap Return, and Causes} \label{ss:priv-entry}
...
Three details of Table \ref{t:priv-causes} are worth stating explicitly.
...
\subsection{Waiting for an Interrupt} \label{ss:priv-wfi}
...
\subsection{User Mode} \label{ss:priv-umode}
...
\subsubsection{What Traps in User Mode}
...
\subsubsection{Machine-Mode Accesses with User Privilege: \bitfield{MPRV}}
...
\subsubsection{What User Mode Does and Does Not Protect} \label{sss:priv-umode-safety}
...
\subsection{Physical Memory Protection} \label{ss:priv-pmp}
...
\subsubsection{The Entry Registers}

Each entry is one 8-bit configuration byte and one address register. The configuration bytes are packed four to a word in \register{pmpcfg0} through \register{pmpcfg3} (addresses \texttt{0x3A0}--\texttt{0x3A3}), entry $i$ occupying byte $i \bmod 4$ of word $\lfloor i/4 \rfloor$; the addresses live in \register{pmpaddr0} through \register{pmpaddr15} (addresses \texttt{0x3B0}--\texttt{0x3BF}). The register map is always the full 16-entry set: on a configuration with fewer entries the surplus registers remain legal registers that read zero and ignore writes, so the same driver code runs on either. All of them reset to zero, which means every entry is off and nothing is locked.
...
\subsubsection{Matching and Permission}
...
\subsubsection{Faults, and the Pre-Issue Guarantee}
...
\subsubsection{A Typical Configuration}
