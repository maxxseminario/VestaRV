...
read-then-write pairs a parallel bus would use.
...
\textbf{The DMI data register} is 41 bits, laid out as
\bitfield{op}\texttt{[1:0]}, \bitfield{data}\texttt{[33:2]},
\bitfield{address}\texttt{[40:34]}. On the way \textit{in}, \bitfield{op} is
...
\begin{itemize}
	...
	\item \textbf{The entry stub is not yet resident in silicon.} The code the hart runs when it enters debug mode occupies \texttt{0x10780} onwards, and in the present revision it is placed there by the verification environment rather than by the chip itself. \textbf{A production part therefore has nothing at that address until something writes it}, and the mechanism by which a shipped chip will place it --- from the boot ROM, from the Debug Module itself, or from a dedicated ROM --- is not yet fixed. Until it is, halting a hart on real silicon requires the entry page to be populated first.
