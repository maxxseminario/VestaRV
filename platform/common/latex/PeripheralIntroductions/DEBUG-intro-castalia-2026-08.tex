...
% NO \resizebox: the emitter draws this graph in printed centimetres and sizes
% the state boxes, the type and the air between them independently (see the
% geometry block in GenerateTapStateDiagram). A \resizebox here would take that
% back over: it scales the boxes, the labels and the gaps by one factor, so
% a layout with more air in it would simply be squeezed until it had less.
\begin{figure}[htpb]
	\centering
...
\rowcolor{tablehighlightcolor} \texttt{0x12} & \register{hartinfo} & Reads \texttt{0x00000000}: no scratch registers, no debugger-visible \register{data0} address. Read-only, and the zero is deliberate; see below. \\
...
\textbf{\register{hartinfo} reads zero on purpose, and a debugger should want it
to.} The register exists to advertise per-hart scratch registers and the address
of \register{data0}, so that a debugger can use them directly. This
implementation advertises neither: its \texttt{nscratch}, \texttt{dataaccess},
\texttt{datasize} and \texttt{dataaddr} fields are all zero. With
\texttt{dataaccess}~$=0$ a conforming debugger does not consult
\texttt{dataaddr} at all and reaches memory through the program buffer and its
own work area instead, which is the only path this chip supports anyway (there is
no System Bus Access; see Section \ref{ss:debug-limits}). Advertising an address
here would also have been actively wrong: the field is twelve bits wide, and the
real \register{data0} address does not fit in it. A truthful zero is better than
a truncated pointer into somebody else's memory.
...
\begin{tabular}{ l p{9.6cm} }
	...
	\rowcolor{tablehighlightcolor} \texttt{0x106F0}, \texttt{0x106F4} & The entry stub's private repair copies of \asminline{s0} and \asminline{s1}. They are not where a debugger reads those registers from; see below. \\
...
(Section \ref{s:multicore}).

And the debug system needs two working registers of its own, which is why
\asminline{s0} and \asminline{s1} are handled differently from every other
register. While a hart is halted, \textbf{the single home of the interrupted
program's \asminline{s0} and \asminline{s1} is the pair of CSRs
\register{dscratch0} and \register{dscratch1}}. A debugger that reads
\asminline{s0} gets the value out of \register{dscratch0}; one that writes
\asminline{s0} writes \register{dscratch0}; and code in the program buffer that
changes \asminline{s0} has that change survive to the debugger's next read,
because the exit sequence adopts the architectural pair into
the \register{dscratch0}/\register{dscratch1} pair before it does anything else. The two words at
\texttt{0x106F0} and \texttt{0x106F4} are the entry stub's own repair copies,
used to rebuild that pair if a hart re-enters debug mode from inside
the page; the Debug Module writes them only when it is itself the author of a
new value, and nothing else should read them at all. The visible consequence is
the one that matters: \textbf{you see the interrupted program's values, not the
stub's working values}, and a plain halt-and-resume with no debugger register
writes leaves the program's \asminline{s0} and \asminline{s1} exactly as it
found them.
...
The sequences above are at the DMI level. In practice a debugger generates them,
and this part has been brought up end to end against stock, unmodified tools:
\textbf{OpenOCD 0.12.0} and \textbf{\texttt{riscv-none-elf-gdb} 13.2}. Nothing in
either was patched. Four facts about that combination are worth knowing before
the first session, because each of them is invisible until it costs an afternoon.

\paragraph{Clock the adapter at 3\,MHz or below.} The Debug Transport Module
reports \bitfield{idle}~$=7$, meaning it needs seven Run-Test/Idle cycles after a
DMI scan before the result is ready. A debugger that honours that field may clock
TCK at up to the port's rated \SI{7.14}{\mega\hertz}. \textbf{Stock OpenOCD does
not honour it}: it learns the required delay by observing failures, starting from
zero. What sets the safe speed is then the requirement that three TCK periods
cover one DMI transaction's time in the module's own clock domain,
$3\,T_{\mathrm{TCK}} \ge t_{\mathrm{DM}}$, which gives
...
The program-buffer figure is the one that binds, because every memory access and
every software breakpoint goes through the program buffer, and it shrinks further
when other harts are contending for the shared bus. \textbf{Set the adapter to
\SI{3}{\mega\hertz} or below}; \SI{3.60}{\mega\hertz} is a hard bound, not a
target. The \SI{7.14}{\mega\hertz} rating applies only to a master that honours
\bitfield{idle}.

\paragraph{\texttt{reset halt} succeeds and resets nothing.} The Debug Module has
no system reset (\bitfield{ndmreset} is unimplemented), so the command degrades:
it asserts halt on every hart, re-reads the ID code, and \textbf{reports
success}. Nothing is reset: memory keeps its contents, harts keep their
execution state, and a running hart's counters carry on from where they were
rather than restarting. The failure is silent, so do not infer a reset from the
absence of an error. Where a genuine halt-on-reset is needed, arm
\bitfield{setresethaltreq} for the hart and power-cycle its tile through the
power controller (Section \ref{s:multicore}).

\paragraph{That recovery works for harts 1 to $N-1$ only.} The per-tile power
controller is the lever behind every recovery procedure here: the wedged-first-word
case of Section \ref{ss:debug-limits}, and halt-on-reset above. \textbf{Hart 0 has
no such lever}: it is always on, its power-controller bit ignores writes, and
\bitfield{ndmreset} and \bitfield{hartreset} read zero. The only thing that
resets hart 0 is the \pin{resetn} pin, which also resets the Debug Module and
therefore ends the debug session. Plan accordingly when choosing which hart to
experiment on.

\paragraph{An \asminline{EBREAK} written into the program buffer becomes a jump.}
Debuggers terminate short program-buffer sequences with an explicit
\asminline{EBREAK}. On this Debug Module a program must instead leave through the
common exit sequence, which is what signals completion and restores
\register{dscratch0} and \register{dscratch1}; an \asminline{EBREAK} that bypasses it would be reported as
an exception even though the work had been done. So a DMI write of exactly the
32-bit \asminline{EBREAK} encoding into \register{progbuf0} or \register{progbuf1}
stores a jump to that exit sequence instead. Reading the word back returns the
jump, not the \asminline{EBREAK}; the substitution is not hidden. Only that
exact encoding in those two registers is affected: \register{data0} stores
whatever it is given, and a word one bit away from \asminline{EBREAK} is stored
verbatim. A program that genuinely traps still never reaches the exit sequence
and is still reported as an exception, so this does not blunt error reporting.
...
\begin{itemize}
	...
	\item \textbf{Debugger support exists, and its rough edges are known.} Stock OpenOCD 0.12.0 and GDB 13.2 attach, enumerate every hart, halt and resume individually, read and write registers and memory, set software breakpoints and single-step (Section \ref{ss:debug-connect}). Two limits are worth stating with it. The adapter must be clocked at \SI{3}{\mega\hertz} or below, for the reason given in that section. And \textbf{bulk memory writes are not yet usable}: \register{abstractauto} is unimplemented, and a debugger that assumes it, as OpenOCD's burst-write path does, writes only the first word of a block and silently drops the rest, which is why loading a program image over the debug port does not currently work. One word at a time is correct; a block is not. Single-stepping is also slow enough that data watchpoints implemented by stepping are impractical.
