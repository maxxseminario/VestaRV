% TRNG chapter. Section order is the per-peripheral template enforced by
...
\begin{enumerate}
	\item Program \register{TRNGROSEL} (0000 selects every ring) and \register{TRNGDECIM} (nonzero, so consecutive samples accumulate independent jitter), and set the health-test cutoff \register{TRNGRCTC} if the hardware default of 32 is not wanted.
	\item Set \register{TRNGEN}. The rings start oscillating and the decimator, assembler and health test start.
	\item Poll \register{TRNGDRDY}, or enable \register{TRNGDRDYIE} and take the interrupt.
	\item Read \register{TRNGxDR}. The read returns the word and consumes it in the same access.
	\item On \register{TRNGALMF}, discard the harvest and write 1 to the flag to clear it. Harvesting stays halted until it is cleared.
\end{enumerate}
