% Event fabric chapter. Section order is the per-peripheral template enforced by
...
% is unchanged apart from the headings it now sits under, and the two table
% captions moved out of their tabular (they broke the List of Tables numbering).
\section{Overview}
...
This version spends no interrupt vector at all: \texttt{irq\_evfab} is wired to a constant 0 and the \register{EVFIE} slot is reserved and reads 0, so \peripheral{EVFAB} never appears in the \peripheral{IRQROUTER}'s vector list and the frozen interrupt-vector numbering is completely undisturbed by this peripheral. Firmware polls \register{EVFSR}, whose two bits are live OR-reductions of \register{EVFFIRED} and \register{EVFOVR}. A future version that spends a vector for \register{EVFFIRED}/\register{EVFOVR} would be a purely additive change (implement \register{EVFIE}, drive \texttt{irq\_evfab}, and route the new vector) with no register-map break.
