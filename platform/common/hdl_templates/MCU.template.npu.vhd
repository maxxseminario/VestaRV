...
-- (A2/Argus: carved out so a config with peripherals.npu=false emits an
-- MCU.vhd with no NPU, no staging RAM and no 0xC000 window). Each block
-- is spliced verbatim at its @GEN marker when the NPU is present.
