	...
	def GenerateChipConfigurationSection(self):
		...
		if classes:
			...
			s += ('\\caption{Per-hart-class ISA of this build. Each row is a hart class the '
				...
				'read-only \\register{misa} CSR. A minimal tile class is rv32iac and nothing '
				'more: the generated \\texttt{TILE\\_ENABLE\\_*} constants drop M, Zb, every '
				'Z-series extension, U-mode and PMP, while A, C, the trap CSRs and debug are '
				'kept (the tiles run the shared-fabric LR/SC and AMO locking, and the Debug '
				'Module halts them). Zicntr, where the chip has it, is kept on both classes '
				'because \\texttt{cycle} and \\texttt{instret} exist on every hart '
				'unconditionally and the knob gates no hardware. Code that more than one class '
				'executes must be built for the '
