...
def emitString(block, packageName=None, spec=None):
    ...
    spec = spec or {}
    # `periph` names the rdl_vs_vhdl reader for this block. RTL_PACKAGES states
    # it (None where there is no reader); the untracked out/rdl/ emissions, which
    # pass no spec, fall back to the addrmap name, which is that key for every
    # house-style block.
    periph = spec['periph'] if 'periph' in spec else block.Name
    ...
    if periph:
        L.append('--                //platform/common:rdl_vs_vhdl_' + periph + '_test')
    else:
        L.append('--                (no rdl_vs_vhdl reader: this block is not memory mapped)')
    L.append('--')
    ...
    if variants:
        L.append('--')
        L.append('-- CONFIGURATION-DEPENDENT BLOCK. Its register set is a function of a generic,')
        L.append('-- so this file carries only what is identical at every shipped configuration:')
        L.append('--   ' + spec.get('variantNote', ''))
        L.append('-- Everything the configurations disagree on -- a word offset computed from a')
        L.append('-- generic, a field whose width tracks the hart count -- is absent here and')
        L.append('-- stays a generic in ' + os.path.basename(spec.get('rtl', '')) + '.')
    L.append('-- =============================================================================')
    L.append('')
