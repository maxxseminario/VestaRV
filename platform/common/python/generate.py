...
def _buildPackageData(model):
	...
	# THE BALL-NUMBER GATE. A model built from a die-row pad list (the overlay attaches one with
	# package.AttachDieRow) is two descriptions of one ring, and the five older cross-checks in
	# that branch compare pad counts, net-name sets and the bonded/unbonded split -- none of them
	# compares a BALL NUMBER, which is why a transposed supply pair survived both files and the
	# emitted pad list. This one compares every die-row pad's ball against the model and against
	# the pad list the model emits, and it is FATAL: a crossed pair is a bonding diagram that
	# shorts two rails together, and it is cheaper to fail here than at the bonder.
	# Models with no die row attach none and this is a no-op.
	_ballMapComplaints = package.CheckDieRowBallMap()
	if _ballMapComplaints:
		raise Exception('package model "' + model + '": the die row and the ball map disagree on '
			+ str(len(_ballMapComplaints)) + ' pad(s). Ball numbers are the bonding diagram, so fix '
			+ 'the two files together rather than either one alone:\n  '
			+ '\n  '.join(_ballMapComplaints))
	return package
...
# here every pad the model will ever carry is present and nothing is skipped.
_ballMapComplaints = m.Package.CheckDieRowBallMap()
if _ballMapComplaints:
	raise Exception('package model "' + packageModel + '": the die row and the finished pad ring '
		+ 'disagree on ' + str(len(_ballMapComplaints)) + ' pad(s):\n  ' + '\n  '.join(_ballMapComplaints))
...
# (PadRing side, Flavor-A side name). The geometric pin order within a side is
# Package.PadListRows': W and N descending (pin 1 at the top of the left edge, +y order
# 11..1; 34 at the right of the top edge, +x order 44..34), S and E ascending.
_sideGeom = [
	('W', 'left'),
	('S', 'bottom'),
	('E', 'right'),
...
# Rows, instance placeholders and emission order all come from Package.PadListRows, which the
# die-row ball-number gate reads too: one derivation, so a ball cannot read one way here and
# another in the check. It also uniquifies repeated placeholders -- a multi-pair supply model
# emits PAD_VDD three times, which is an illegal duplicate instance name in the consuming
# netlist and padlists -- giving a base that occurs once its bare name (PAD_RESETN, PAD_P3_0)
# and repeats _0/_1/... in emission order, so the names line up with the staged netlist.
_padRows = m.Package.PadListRows()
for _sd, _sideName in _sideGeom:
	_padTclLines.append('set PADRING_' + _sideName.upper() + ' {}')
	for (_rowSide, _p, _inst) in _padRows:
		if _rowSide != _sd:
			continue
		if _inst is None:
