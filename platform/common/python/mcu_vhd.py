...
# Shim peripherals whose select strobe is re-registered on falling mclk before it
# reaches the peripheral's active-low en (emitPolarityShims emits the
# snapshot_strobe_reg process and the shslv_<sel>_en_q signals; I3C0 and NFC0 carry
# the same flop inside their own instance regions). A negative-edge flop on mclk is a
# clock on a clock pin, which is allowed; the rule the Z wave enforces is that no DATA
# signal reaches a flop clock pin.
# History, because the reason changed under the set. Before 2026-09-15 these blocks
# clocked status and RX snapshot registers on falling_edge(en_mem), and the shim kept
# a combinational en AND decode glitch out of that capture clock. Z1 and Z2b removed
# every such falling-edge capture (work.sync plus toggle-qualified copies, read by
# periph_regs' ClkMem read register), and Z1 then emptied this set. Z4 bisected the
# topology-B zero-delay gate regression (0 of 14 TIMEOUT against 13 of 14) to exactly
# that change, netlist out.z4bx_20260915.
# The surviving mechanism is the one Z1's own note named: the arbiter's s_en
# self-clears on the same rising mclk edge at which the slave samples EnMemPeriph, so
# a raw-strobe shim deasserts the peripheral's en in the same delta as its own capture
# edge. With zero delay the deassertion wins and the access is lost; real cell delays
# hide it, which is why the same netlist is 14 of 14 under SDF. The falling-mclk
# re-register moves both edges half a cycle away from every rising-edge sampler, so
# the strobe is stable across the edge that consumes it. It costs 11 flops.
# See hdl/common/regs/REGFILE.md, "A held strobe is only as wide as the fabric's
# select". Do not empty this set again without a gate-level zero-delay run behind it.
...
AFE_SLOT12_INSTANCES = [
	"    -- AFE register stubs + shared EIS engine stub: four AFE sites subdivide page-0 slot 12 (0x4C00) into 64 B sub-slots (sub-slot = sh_addr(5:4)), each answers only for its owner hart OR hart 0, and the EIS engine is hart-0-only in the IRQ-router page top quarter (0x7C00-0x7FFF, where irq_router's ADDR_W=10 decode is inert); a denied read returns 0, a denied write is dropped, and every stub resets all-zero.",
...
class McuVhdEmitter():
	...
	def emitSlot12Instances(self):
		...
		if self.qspi:
			return [
				'    -- QSPI0: Quad-SPI flash controller, page-0 slot 12 (0x4C00), smclk-domain serial core (SYS_CLK_CR=0 rule), registered read with no RX side effects, vectors 55 and 56; pins unbonded.',
	...
	def afeStubsOrchOwners(self):
		...
		for ln in AFE_SLOT12_INSTANCES:
			...
			if 'its owner hart OR hart 0' in ln or 'is hart-0-only' in ln:
				...
				ln = ln.replace('is hart-0-only', 'is hart-0-only (the orchestrator)')
			out.append(ln)
		return out
	...
	def emitI3cInstance(self):
		...
		return [
			...
			'    -- I3C registers its read on rising ClkMem, and the raw select deasserts in the same delta as that edge, so it takes the FALLING-MCLK re-registered strobe (see snapshot_strobe_reg).',
	...
	def emitNfcDecls(self):
		...
		if not self.nfc:
			return []
		return [
			'        /* NFC0 (ISO 14443A tag / card-emulation engine), page-2 (MUTEX page) sub-slot 2 @0x6200: registered-read shim (no bridge), active-low one-cycle en off the falling-mclk re-registered strobe.',
	...
	def emitNfcInstance(self):
		...
		lines = [
			...
			'    -- NFC registers its read on rising ClkMem and its W1C clears generate their own ClkMem level, so there is no falling_edge(EnMemPeriph) pre-latch left; the en still comes off the FALLING-MCLK re-registered strobe, which keeps the select stable across the sampling edge (see snapshot_strobe_reg).',
	...
	def emitRtcInstance(self):
		...
		lines = [
			'',
			'    -- RTC0: 32.768 kHz always-on real-time clock, MUTEX-page sub-slot 5 @0x6500, zero pins; the 47-bit count, alarm compare and tick counter ride the UNGATED lfxt_in crystal so neither SYS_CLK_CR nor PWRCTRL can stop them, while clk takes mclk for the CDC, the W1C flags and irq_rtc (vector 114).',
	...
	def emitPwmInstance(self):
		...
		lines = [
			'',
			'    -- PWM0: 2-channel buffered PWM generator, MUTEX-page sub-slot 6 @0x6600, zero input pins; counter and output stage ride the free-running MCLK, PER/DTY0/DTY1 writes are double-buffered and commit at the period boundary, FLTTRIG+FLTEN forces both outputs safe within one clock, irq_fault vector 115 and irq_evt vector 116.',
			'    -- PLAIN raw active-low en strobe (the GPIO4/5 native-slave idiom), so NO falling_edge(EnMemPeriph) pre-latch and NO capture-clock en_q here.',
	...
	def emitOwInstance(self):
		...
		return [
			'',
			'    -- OW0: 1-Wire master, MUTEX-page sub-slot 7 @0x6700, standard + overdrive; the OW0DIV time base and slot FSM ride the free-running MCLK, OW_DQ_IN is 2-FF synchronized PURE DATA and never a clock, irq_ow drives vector 117, and the one open-drain DQ pad is P4.7/GPIO31 alt plane AF2.',
			'    -- PLAIN raw active-low en strobe (the GPIO4/5 native-slave idiom), so NO falling_edge(EnMemPeriph) pre-latch and NO capture-clock en_q here.',
	...
	def emitDmaInstance(self):
		...
		lines = [
			'',
			'    -- DMA0: multi-channel single-shot DMA, MUTEX-page sub-slot 8 @0x6800, zero pins; it is both an arbiter slave (register file on ClkMem) and the ' + np1 + 'th arbiter master, slicing into arb_*(' + ns + ') at boundary depth 0 on mclk, never grant-holding (lock tied \'0\', lrsc "00"); the three trigger inputs are the only true CDC and irq_done/irq_err drive vectors 118 and 119.',
	...
	def emitI2ctInstance(self):
		...
		lines = [
			'',
			"    -- I2CT0: hardware-autonomous I2C target, MUTEX-page sub-slot 10 @0x6A00; the whole target rides the free-running MCLK and SDA_IN/SCL_IN are 2-FF synchronized PURE DATA, unlike I2C0's SCL-pad-clocked FSM; no new pins, it shares I2C0's open-drain SDA0/SCL0 planes, and vectors 122 and 123 carry address/error and tx-ready/rx-full.",
			'    -- PLAIN raw active-low en strobe (the GPIO4/5 native-slave idiom), so NO falling_edge(EnMemPeriph) pre-latch and NO capture-clock en_q here.',
	...
	def emitTrngInstance(self):
		...
		lines = [
			'',
			'    -- TRNG0: ring-oscillator entropy source + harvest engine, MUTEX-page sub-slot 9 @0x6900, both instances sharing the NRO generic (' + nro + '); the harvest engine rides the free-running MCLK, TRNG0DR is the ONE read-CONSUMES register in this library, irq_trng drives vector 121, and there are zero pins (the source is the sibling u_ro instance).',
			'    -- PLAIN raw active-low en strobe (the GPIO4/5 native-slave idiom), so NO falling_edge(EnMemPeriph) pre-latch and NO capture-clock en_q here.',
	...
	def emitEvfabInstance(self):
		...
		lines = [
			'',
			'    -- EVFAB0: event/trigger fabric, MUTEX-page sub-slot 11 @0x6B00; a PPI-style crossbar of 8 {EVSEL, TASKSEL} channels turns one of 16 events into a one-mclk pulse on one of 10 tasks with every hart in WFI, is never a bus master, owns all the CDC for its taps, and is VECTORLESS, so irq_evfab is constant \'0\' and left open.',
			'    -- PLAIN raw active-low en strobe (the GPIO4/5 native-slave idiom), so NO falling_edge(EnMemPeriph) pre-latch and NO capture-clock en_q here.',
		...
		if self.dma:
			...
			lines.append("    evfab0_task_busy(0)  <= '0';                      -- T0 DMA0 ch0 GO, block absent")
			lines.append("    evfab0_task_busy(1)  <= '0';                      -- T1 DMA0 ch1 GO, block absent")
		...
		if self.npu:
			...
			lines.append("    evfab0_task_busy(6)  <= '0';                      -- T6 NPU0 THINK, block absent")
	...
	def emitEvfabTaps(self, instKey):
		...
		taps = {
			...
			'gpio0': [
				'            -- EVFAB taps: the pre-mask pad-edge export (EV15 front-end) and the two output tasks, acting on the pins selected by P1TASK (T7/T8).',
			...
			'timer0': [
				'            -- EVFAB taps: producer toggles EV4/EV5 (timer_clock domain, the fabric does the 2-FF + XOR) and the START/STOP tasks (T2/T3).',
			...
			'npu0': [
				'            -- EVFAB task (T6): one-mclk THINK launch, ORed with the NPUCR bit-16 write path.',
			...
			'pwm0': [
				'            -- EVFAB taps: period boundary + fault SET condition (EV2/EV3), and the fault-trip task (T4, still FLTEN-gated inside the block).',
				'            evt_period  => evfab0_pwm0_period,',
				'            evt_fault   => evfab0_pwm0_fault,',
				'            task_flttrig => evfab0_task_pulse(4),',
			],
			'nfc0': [
				'            -- EVFAB taps: field-detect / rx-frame TOGGLES (EV8/EV9).',
				'            -- NFC0 runs on smclk, so these are toggle exports and the fabric owns the CDC.',
				'            evt_field    => evfab0_nfc0_field,',
				'            evt_rxframe  => evfab0_nfc0_rxframe,',
			],
			'dma0': [
				'            -- EVFAB taps: the channel GO tasks (T0/T1 in bits 0/1, channels 2/3 tied \'0\'), the done/err event pulses (EV10/EV11/EV12) and the per-channel busy levels the fabric samples for OVR.',
			...
			'pwr0': [
				'            -- EVFAB task (T5): one-mclk tile-wake pulse.',
				'            -- Policy stays in PWRCTRL (the pulse clears gate_req under PWRWAKE\'s mask); the boot-gate release is deliberately NOT a task.',
	...
	def emitGpio4Instance(self):
		...
		head = [
			'',
			'    -- GPIO4: general-purpose I/O port 5, MUTEX-page sub-slot 3 @0x6300, per-pin IRQs on vectors 98-105; AF0 = plain GPIO, AF1 = QSPI0/I3C0 pin functions (Hi-Z when absent).',
		]
	...
	def emitGpio5Instance(self):
		...
		head = [
			'',
			'    -- GPIO5: general-purpose I/O port 6, MUTEX-page sub-slot 4 @0x6400, per-pin IRQs on vectors 106-113; AF0 = plain GPIO, AF1 = NFC0 digital-AFE pins (Hi-Z when absent).',
		]
	...
	def emitPolarityShims(self):
		...
		if capture:
			lines += [
				ind + '-- SPI/UART/TIMER/I2C (and QSPI when configured) take a FALLING-MCLK re-registered select strobe, not the raw combinational decode. The arbiter clears s_en on the same rising mclk edge at which the slave samples EnMemPeriph, so the raw strobe deasserts in the same delta as the edge that consumes it, and the combinational decode also glitches in the skew window just after that edge.',
				ind + '-- Re-registering on the falling edge moves both the assertion and the deassertion half a cycle away from every rising-edge sampler. Removing it cost the whole topology-B zero-delay gate regression (Z4, out.z4bx_20260915); with real cell delays the same netlist passes either way.',
	...
	def i2c1Block(self, name):
		...
		if not self.i2c1:
			if name == 'i2c1-instance':
				return ['    -- I2C1 dropped by this configuration (window slot 15 reads zero; vectors 70-82 are reserved; the SDA1/SCL1 pad planes are hi-Z)']
			return []
	...
	def uart1Block(self, name):
		...
		if not self.uart1:
			if name == 'uart1-instance':
				return ['    -- UART1 dropped by this configuration (window slot 5 reads zero; vectors 52-54 are reserved; the TX1/RX1 pad planes are hi-Z)']
			return []
	...
	def spi1Block(self, name):
		...
		if not self.spi1:
			if name == 'spi1-instance':
				return ['    -- SPI1 dropped by this configuration (window slot 3 reads zero; vectors 11-12 are reserved; the CS1/MISO1/MOSI1/SCK1 pad planes are hi-Z)']
			return []
	...
	def timer1Block(self, name):
		...
		if not self.timer1:
			if name == 'timer1-instance':
				return ['    -- TIMER1 dropped by this configuration (window slot 7 reads zero; vectors 22-27 are reserved; the T1CMP0/T1CMP1/T1CAP0/T1CAP1 pad planes are hi-Z)']
			return []
	...
	def emitShWindowConst(self):
		...
		lines.append(ind + '   RomAddrBits is the word-address width RomSize asks for; RomMacroAddrBits is the width the rom2k_hvt_pg macro at rom0 actually has, and the two are checked against each other there.')
		...
		lines.append((ind + 'constant RomMacroAddrBits : natural := ' + str(ROM_MACRO_ADDR_BITS) + ';').ljust(78)
			+ '-- the rom2k_hvt_pg macro is ' + str(1 << ROM_MACRO_ADDR_BITS) + ' x 32')
	...
	def emitMemslvDecls(self):
		...
		lines.append(ind + '-- boot ROM + bulk RAM banks' + (' + NPU staging RAM' if self.npu else '') + ' are hard macros: their Q is the 1-cycle registered read the arbiter\'s slave model expects, so the macro output IS the rdata (no extra register).')
		lines.append(ind + '-- Enables/WEN are ACTIVE-LOW at the macro; see the shims below.')
		def vdecl(name):
	...
	def emitTcmApertureDecls(self):
		...
		n = len(apx)
		lines = ['']
		lines.append(ind + '-- Read-only TCM apertures: one 16 KiB window per hart at 0x20000 + 0x4000*h (h = 0..' + str(n - 1) + ') through which hart 0 reads any hart\'s private TCM. The aperture address is a TCM WORD index and only sh_addr(10:0) reaches the tile, so window word i is that hart\'s byte address 0x8000 + 4*(i mod 2048) and the upper half of every window MIRRORS the lower. Three gates answer with ZERO rather than a bus error, a stall or a hang (sh_master /= 0, a write, a power-gated target whose iso clamp zeroes tcm_ext_done as well as rdata), so software checks PWRSR first because zero is a legal TCM value; this is the only slave that stalls the arbiter, holding s_stall for the 6 mclk a read takes.')
		for h in range(n):
		...
		lines.append(ind + '-- The aperture sequencer, and there is exactly ONE of it: the arbiter serializes, so only one aperture transaction can be in flight.')
	...
	def emitTcmApertures(self):
		...
		zeros = "(others => '0')"
		lines = ['']
		lines.append(ind + '-- Read-only TCM aperture sequencer (the declarations above carry the map, the three gates and the s_stall argument).')
		for h in range(n):
		...
		lines.append(ind + '-- LAUNCH = addressed AND the management hart AND a read AND the target is alive.')
		lines.append(ind + '-- Anything else is answered with zero in the same three cycles every other slave takes.')
		...
		lines.append(ind + '-- The stall must already be asserted in the cycle the enable strobe occupies (the arbiter samples s_stall at the edge that would otherwise take it to DATA), so it is COMBINATIONAL on launch and registered only afterwards, through busy.')
		lines.append(ind + 'tcmw_stall'.ljust(16) + ' <= tcmw_launch or tcmw_busy;')
		lines.append(ind + '-- Completion / abort of the IN-FLIGHT transaction (tcm_ext_req is one-hot while busy, so these are single-term ORs, not priority chains).')
	...
	def emitSharedRamBanks(self):
		ind = ' ' * 4
		lines = []
		lines.append(ind + "-- Shared bulk RAM = %d x sram1p16k macros (%d KB, 0x10000-0x%05X). The macro IS the arbiter's slave model: CEN sampled with the address at the s_en cycle's ending edge and Q valid the next cycle (1-cycle registered read), enables and WEN ACTIVE-LOW at the macro against the arbiter's active-high strobes, and WEN taken from the resv-GATED sh_we per byte lane so a suppressed SC write cannot touch memory. No INIT, so power-up contents are undefined on silicon and zeroing the mailboxes before any hart reads them is a bootrom obligation."
...
def generateMcuVhd(gen, templatePath, outPath):
	...
	header = []
	header.append('/* VestaRV: MCU top level (' + str(emitter.nHarts()) + (' hart' if emitter.nHarts() == 1 else ' harts') + ', MCU_MP)')
	header.append('   Fixed regions come from hdl_templates/MCU.template.vhd, the configuration-driven regions from python/generate.py and python/mcu_vhd.py.')
	header.append('   Generated on ' + generatedOnStamp() + ' by platform/common (make chip).')
	header.append('   Do not edit this file: edit the template or the emitter, then regenerate. */')
	header.append('')
