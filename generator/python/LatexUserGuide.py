...
class LatexUserGuide():
	...
	SaveDirectory = None	# {chip root directory}/latex/MCU-User-Guide
	...
	def CopyTemplateTexFiles(self):
		...
		copyfile(path, self.SaveDirectory + '/MCU-User-Guide.tex')
	...
	def GenerateAddressSpaceDiagram(self):
		...
		for i, ramSlot in enumerate(self.Gen.RamMemorySlotsUsed):
			...
			addr = self.Gen.RamStartAddress + ((ramSlot - 2) * self.Gen.RamMemorySlotSize)
		...
		if self.Gen.NativeSpiFlashMemoryReadAccess and not self.Gen.NativeSpiFlashMemoryWriteAccess:
			s += '\\\\\n'
			s += '\\memsection{' + fmthex(slots[-1][3] + 1, minDigits=7) + '}{' + fmthex(0x0FFFFFF, minDigits=7) + '}{3}{\\textit{\\color{lightgray}Unused}} \\\\\n'
	...
	def GeneratePeripheralSections(self):
		...
		for pt in pts:
			...
			for rt in pt.RegisterTemplates:
				...
				s += '\\subsubsection{\\texttt{' + fmttex(displayRegisterName) + '} Register} \\label{ss:' + fmttex(rt.NameTemplate) + '}\n'
	...
	def GeneratePeripheralAndRegistersList(self):
		...
		for p in self.Gen.Peripherals:
			...
			for r in p.Registers:
				if thisRowColored:
					s += '\\rowcolor{tablehighlightcolor} '
				s += '\\hyperref[ss:' + fmttex(r.Template.NameTemplate) + ']{\\texttt{' + fmttex(r.Name) + '}} & '
