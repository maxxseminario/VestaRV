...
# vestarv: resetn is a true asynchronous reset (every FF in the core uses
# process(clk, resetn) async clear). OpenSTA otherwise times reset-assert->Q
# propagation into setup paths (fanout ~1900 from the port), swamping WNS
# with a non-path. Slew/cap repair still buffers the net.
if { [llength [get_ports -quiet resetn]] == 1 } {
