...
# MINIMAL-ISA CORNER TILE (2026-08-16, USER: rv32iac tiles for area and power).
# This macro is what harts 1..N-1 are, and MCU.vhd wires those instances with
# TILE_ENABLE_MUL/DIV/BITMANIP = false. The macro MUST agree, and NAMING THEM
# HERE IS THE ONLY WAY TO MAKE IT SO: hart_tile.vhd's ISA generics default to the
# FULL ISA, a tile-only elaborate takes those defaults, and MemoryMap constants
# reach the generics only through MCU.vhd -- which does not exist in this run.
# That is exactly how ENABLE_DEBUG silently produced a debug-OFF macro earlier in
# this flow; the guard below is the lesson applied.
# ONLY M AND B ARE DROPPED. ENABLE_ATOMICS and ENABLE_COMPRESSED stay at their
# true defaults: these tiles run the shared-fabric LR/SC + AMO locking, and C
# shrinks code, which matters at an 8 KiB TCM.
set TILE_ENABLE_MUL      false
set TILE_ENABLE_DIV      false
set TILE_ENABLE_BITMANIP false

elaborate $TOP_MODULE -parameters [list [list SH_AW $PENTA_SH_AW] \
                                       [list ENABLE_DEBUG $PENTA_ENABLE_DEBUG] \
                                       [list ENABLE_MUL $TILE_ENABLE_MUL] \
                                       [list ENABLE_DIV $TILE_ENABLE_DIV] \
                                       [list ENABLE_BITMANIP $TILE_ENABLE_BITMANIP]]
...
# MINIMAL-ISA GUARD, the SH_AW/debug guard's sibling. hart_tile.vhd wraps the
# divider in `gen_div.divider`, so a div-ON elaborate has instances under that
# path and a div-OFF one has none. Fails CLOSED: if ENABLE_DIV did not take, the
# macro is a FULL-ISA tile while MCU.vhd wires it minimal -- the M14
# netlist-boundary class again, and worth ~23,000 um2 per tile of silent bloat.
set N_DIV [llength [get_db insts -if {.name =~ "*gen_div*"}]]
puts "### UNL STATUS ### : divider instances = $N_DIV (expect 0 for ENABLE_DIV=$TILE_ENABLE_DIV)"
if {!$TILE_ENABLE_DIV && $N_DIV != 0} {
	puts "FATAL (minimal tile): ENABLE_DIV did NOT take -- $N_DIV divider instances survive."
	puts "  This macro is a FULL-ISA tile; MCU.vhd wires harts 1..N-1 as rv32iac. Do NOT harden it."
	exit 1
}
