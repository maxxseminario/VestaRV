...
set TOP_MODULE       MCU
set BASENAME         MCU_ARGUS_hier.genus
set NUM_HARTS        18
# Argus staging dir: the make-chip N=18 emit (MCU.vhd + MemoryMap.vhd). The
# shared control-plane RTL (clint/arbiter/mutex/...) still comes from MCU_MP/
# (parameterized by the generic maps in the Argus MCU.vhd instantiation).
set ARGUS_DIR        $HDL_DIR/argus
...
# Argus MemoryMap (SH_AW=16, 8 banks, no NPU addr, 32 mutexes, 18-hart CLINT)
read_hdl -vhdl -library work $ARGUS_DIR/MemoryMap.vhd
...
# NPU DROPPED for Argus (no npu0/npuram0 instances in the Argus MCU.vhd).
...
# ARGUS tile: SH_AW=16 (the Argus tile netlist is hart_tile_argus.genus.v; the
# injected dummy parameters must match the Argus MCU.vhd generic map exactly).
exec bash -c "mkdir -p $INPUT_DIR && awk '
	/^module hart_tile\\(/ { inhdr=1 }
	{ print }
	inhdr && /\\);\$/ { print \"  parameter PC_RST_VAL = 32'\\''h00000000;\"; print \"  parameter SH_AW = 16;\"; print \"  parameter ENABLE_MUL = 1;\"; print \"  parameter ENABLE_DIV = 1;\"; print \"  parameter ENABLE_ATOMICS = 1;\"; print \"  parameter ENABLE_COMPRESSED = 1;\"; print \"  parameter ENABLE_BITMANIP = 1;\"; inhdr=0 }
' ../hart_tile_argus/out/hart_tile_argus.genus.v > $INPUT_DIR/hart_tile_argus_top.gen.v"
read_hdl $INPUT_DIR/hart_tile_argus_top.gen.v
...
exec bash -c "sed -e '/=> CORE_ENABLE_/d' \
	...
	$ARGUS_DIR/MCU.vhd > $INPUT_DIR/MCU_argus_top.gen.vhd"
read_hdl -vhdl -library work $INPUT_DIR/MCU_argus_top.gen.vhd
...
# NPU dropped -- no npuram0 in Argus. Shared bulk RAM banks shbank0-7:
for {set b 0} {$b < 8} {incr b} {
	catch { set_false_path -to pin:$TOP_MODULE/shbank$b/PGEN }
}
for {set h 0} {$h < $NUM_HARTS} {incr h} {
...
# NPU dropped -- no npu0/Decision net to dont_touch.
