################################################################################
#
# Genus TCL script -- hart_tile TILE-ONLY synthesis (ARGUS A4 compact-tile flow)
#
# ARGUS VARIANT of tcl/hart_tile.genus.tcl. IDENTICAL synthesis recipe, one
# difference that matters: the tile is elaborated at SH_AW=16 (the Argus
# shared-window width -- 0x0-0x3FFFF, flash from 0x40000), so its sh_addr port
# is [15:0] not [14:0] and adddec's complementary flash decode carves the
# Argus TCM window. The 18 hart instances in the Argus assembly connect to a
# 16-bit-sh_addr arbiter, so the tile netlist MUST match. Outputs carry the
# _argus basename so the Castalia tile artifacts (out/hart_tile.genus.*,
# SH_AW=15, U-shape) are left pristine -- Castalia must stay buildable.
#
# The MODULE stays named plain "hart_tile" (parameter-naming suppressed): the
# Argus hierarchical top instantiates "hart_tile" 18x and resolves against
# this netlist / its LEF. Two files named hart_tile{,_argus}.genus.v each hold
# a module "hart_tile"; they never share an innovus/genus session.
#
...
# the top-level flow places it Nx (the flat flow uniquified the tile into
...
set BASENAME         hart_tile_argus.genus
set ARGUS_SH_AW      16
...
# Elaboration
################################################################################
puts "Elaborating $TOP_MODULE at SH_AW=$ARGUS_SH_AW (Argus shared-window width)"
...
# overridden; PC_RST_VAL + the ENABLE_* ISA switches keep their defaults.
elaborate $TOP_MODULE -parameters [list [list SH_AW $ARGUS_SH_AW]]
...
# HARD GUARD: the SH_AW override must have taken -- sh_addr is a [SH_AW-1:0]
# bus, so 16 bit-ports at the Argus width, 15 at the Castalia default. A wrong
# width here means the whole assembly wires the tiles to the arbiter one bit
# short. Fail fast (this fires seconds after elaborate, before the long synth).
set N_SHADDR [llength [get_db ports sh_addr*]]
puts "### UNL STATUS ### : sh_addr port bit-count = $N_SHADDR (expect $ARGUS_SH_AW)"
if {$N_SHADDR != $ARGUS_SH_AW} {
	puts "FATAL (A4): sh_addr is $N_SHADDR bits, expected $ARGUS_SH_AW -- SH_AW override did NOT take"
	exit 1
}
