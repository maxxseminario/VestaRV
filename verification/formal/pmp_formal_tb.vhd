-- =====================================================================
-- verification/formal/pmp_formal_tb.vhd
-- S4 / R-S4-2: a TRACKED wrapper bench for pmp_unit's grant-function
-- properties, existing for one reason -- to expose a REAL `settled`
-- flag that the PSL properties can qualify on.
--
-- Why not reuse pmp_unit_tb: its `checks` signal is written ONCE in its
-- summary block, so guarding on it restricts every property to the final
-- quiescent state (measured; the witness instrument caught it).  And
-- pmp_unit_tb is left UNTOUCHED on purpose -- the same don't-overload
-- reasoning that put the DS5 guard in its own script.
--
-- `settled` is '0' until the first input pattern has propagated, then
-- '1' for the whole stimulus.  Unclocked PSL is evaluated at time 0
-- where every output is 'U', so this is the combinational analogue of
-- the `resetn = '1'` qualifier the clocked properties carry.
--
-- Instance shape mirrors pmp_unit_tb: 16-entry ON, 8-entry ON, OFF.
-- =====================================================================
...
architecture tb of pmp_formal_tb is
    ...
    signal settled   : std_logic := '0';   -- THE GUARD
    -- SAMPLING CLOCK (R-S4-2, second finding): pmp_unit is combinational,
    -- so its inputs and outputs settle in DIFFERENT deltas.  A continuous
    -- unclocked invariant relating them is transiently FALSE during
    -- settling -- measured: P_UMODE_NOMATCH fired mid-transition while the
    -- DUT was correct (pmp_check returns priv_m on no-match).  `sample`
    -- pulses only AFTER each input change has propagated, giving the
    -- properties the same post-settle sampling the clocked fabric
    -- properties get for free from rising_edge(clk).
    signal sample    : std_logic := '0';
    -- Stimulus-shape flags: they tell the properties which region shape the
    -- current address is in, so a decode property can be written WITHOUT
    -- restating pmp_check (which would be tautological).
...
begin
    ...
    begin
        ...
        -- ==================================================================
        -- R-S4-3 items 1 & 2: DECODE-SENSITIVE stimulus.  The first pass
        -- only ever probed no-match shapes, so a mutated decoder flipped
        -- grants in silence.  These drive matched IN-region and OUT-of-region
        -- addresses so a wrong decode must change an observable grant.
        -- ==================================================================
        -- NAPOT region: base 0x1000, size 16 bytes -> pmpaddr = 0x400|0x1 = word
        -- encoding with one trailing 1.  Region covers 0x1000..0x100F.
        ...
        d_addr <= x"00000FFC"; step;
        -- THE BOUNDARY, a = hi.  RISC-V TOR is [lo,hi) so this is OUTSIDE.
        -- The first pass never probed it, which is how an a<hi -> a<=hi
        -- off-by-one escaped.
