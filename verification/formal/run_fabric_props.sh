#!/bin/sh
# VestaRV: run arb_lat_tb's stimulus with the shared-fabric PSL properties
# bound, at every N_DELAY in $DELAYS.
# Runs from verification/formal/ with explicit paths; no canonical copy under
# the gitignored xcelium/ is needed. rc 0 pass, 1 property failure,
# 2 infrastructure. `set -u` is absent because cdspaths.sh references a
# never-defined assuraPath and would kill the shell before xrun runs.
. ~/vestarv/cdspaths.sh >/dev/null 2>&1
ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
HDL="$ROOT/hdl/common"
FORMAL="$ROOT/verification/formal"
DELAYS=${DELAYS:-"0 1 2"}
# MODE=props expects 0 fires at every depth. MODE=witness binds the witness
# file instead; a witness is `assert never {trigger}`, so a fire is the success
# signal and the run fails if any witness never fired.
MODE=${MODE:-props}
case "$MODE" in
...
for d in $DELAYS; do
    ...
    # The bench reaches a verdict on either line, or it never ran. Only the
