...
set -e
cd "$(dirname "$0")"
SRC=../../../genus/MCU_ARGUS/out/MCU_ARGUS_hier.genus.v
TILE=../../../genus/hart_tile_argus/out/hart_tile_argus.genus.v
...
# A5: same strip for the CONNECTED chip netlist (chip_top + pads + mcu0).
CXSIM=out/chip_top_argus.xsim.v
if [ -f "$CXSIM" ]; then
	strip_mods "$CXSIM" out/chip_top_argus.xsim.clean.v
	echo "chip sim netlist: $(grep -c "^module" out/chip_top_argus.xsim.clean.v) modules kept, $(grep -cE "hart_tile hart" out/chip_top_argus.xsim.clean.v) tile refs -> out/chip_top_argus.xsim.clean.v"
fi
rm -f "$TILE_MODS"
