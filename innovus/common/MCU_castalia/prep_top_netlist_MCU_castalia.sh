#!/bin/bash
# =============================================================================
# prep_top_netlist_MCU_castalia.sh -- POST-run netlist hygiene for the
# MCU_castalia CONNECTED chip cut.
#
# SCOPE, deliberately narrow. The MCU_castalia flow is a FLAT chip run
...
# (out/MCU_castalia.xsim.v). Innovus must see none of the tile modules
...
#
#   -> out/MCU_castalia.xsim.clean.v   (pads-in-DUT gate-sim cell list)
#
# Usage: ./prep_top_netlist_MCU_castalia.sh
#        (run AFTER `make MCU_castalia.innovus`)
...
PNR=../MCU_WOUND/in/MCU_WOUND_hier.pnr.v
WXSIM=out/MCU_castalia.xsim.v
WCLEAN=out/MCU_castalia.xsim.clean.v

[ -f "$TILE" ] || { echo "ERROR: $TILE not found (run 'make hart_tile.genus' in ../../../genus first)"; exit 1; }

if [ ! -f "$PNR" ]; then
	echo "ERROR: $PNR not found -- the MCU_castalia flow consumes it as its"
...
if [ -f "$WXSIM" ]; then
	...
	echo "NOTE: $WXSIM not present yet -- run 'make MCU_castalia.innovus' first,"
