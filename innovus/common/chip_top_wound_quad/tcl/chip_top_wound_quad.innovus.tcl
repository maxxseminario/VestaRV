################################################################################
# Innovus script -- chip_top_wound_quad: the CONNECTED 3x3mm WOUND-PATCH SoC on
...
# chip_top_wound_quad instantiates the tphn pads and `MCU mcu0`, and the whole
...
#      ./prep_top_netlist_wound_quad.sh does the POST-run xsim strip.
#   3. ./gen_chip_top_wound_quad_sdc.sh -> in/chip_top_wound_quad.sdc
...
#  2. DESIGN_NAME + BASENAME = chip_top_wound_quad; init_verilog =
#     in/chip_top_wound_quad.v + in/MCU_WOUND_hier.pnr.v; init_mmmc_file =
...
# WQ-DELTA 2
set DESIGN_NAME chip_top_wound_quad
set BASENAME    chip_top_wound_quad
...
################################################################################
set init_verilog             "$INPUT_DIR/chip_top_wound_quad.v ../MCU_WOUND/in/MCU_WOUND_hier.pnr.v"
...
# Tile timing enters via the ETM .lib (viewdefinition_chip_wound_quad.tcl:
...
#   signoff_mp/gds_via_window.py on out/chip_top_wound_quad.gds2 around
...
# Written to rpt/chip_top_wound_quad.pgcheck.rpt and echoed to the log.
################################################################################
...
pgputs $fh "== chip_top_wound_quad PG verification (CQ3a method) =="
...
if {$RUN_PNR} {
    ...
    # this design, so its geometry is native to the chip_top_wound_quad struct.
...
toc
printStatus "chip_top_wound_quad (wound-patch SoC on the quad floorplan, LQFP-100) complete"
exit
