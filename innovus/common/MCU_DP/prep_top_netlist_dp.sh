...
#
# The top genus run (MCU_DP_hier) reads the TILE GATE NETLIST as source (the
# only shape genus 19.15 accepts for a direct-entity-instantiated block), so
# out/MCU_DP_hier.genus.v contains hart_tile AND its whole submodule tree
...
#                        -> in/MCU_DP_hier.pnr.v  (init_verilog)
# Step 2 (post-innovus): same strip on the emitted sim netlist, if present
#                        -> out/MCU_DP.xsim.clean.v (gate-sim cell list)
#
# Usage: ./prep_top_netlist.sh   (run before make MCU_DP.innovus; run again
...
set -e
cd "$(dirname "$0")"
SRC=../../../genus/MCU_DP/out/MCU_DP_hier.genus.v
TILE=../../../genus/hart_tile/out/hart_tile.genus.v
DST=in/MCU_DP_hier.pnr.v

[ -f "$SRC" ]  || { echo "ERROR: $SRC not found (run 'make MCU_DP_hier.genus' in ../../../genus first)"; exit 1; }
...
XSIM=out/MCU_DP.xsim.v
if [ -f "$XSIM" ]; then
	strip_mods "$XSIM" out/MCU_DP.xsim.clean.v
	echo "sim netlist: $(grep -c "^module" out/MCU_DP.xsim.clean.v) modules kept, $(grep -cE "hart_tile hart[0-3]" out/MCU_DP.xsim.clean.v) tile refs -> out/MCU_DP.xsim.clean.v"
fi
...
# G2: same strip for the CONNECTED DP chip netlist (chip_top_dp + pads + mcu0).
DXSIM=out/chip_top_dp.xsim.v
if [ -f "$DXSIM" ]; then
	strip_mods "$DXSIM" out/chip_top_dp.xsim.clean.v
	echo "dp chip sim netlist: $(grep -c "^module" out/chip_top_dp.xsim.clean.v) modules kept, $(grep -cE "hart_tile hart" out/chip_top_dp.xsim.clean.v) tile refs -> out/chip_top_dp.xsim.clean.v"
fi
