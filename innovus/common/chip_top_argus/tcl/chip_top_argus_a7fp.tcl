...
set BASENAME    chip_top_argus_a7fp
...
saveDesign $DATABASE_DIR/chip_top_argus_a7fp.innovus -def
plog "### UNL A7FP ### : saved $DATABASE_DIR/chip_top_argus_a7fp.innovus (+.def.gz)"

# --- 18-tile placement evidence (dbGet, same session). box_llx/box_lly/orient
# are the same accessors the PG rescue loops use. Asserts x in {30+445c},
# y in {565.085,1268.085,1971.085}, orient R0. The DEF (saved above) carries
# the authoritative COMPONENTS + SPECIALNETS for the python lane cross-check. ---
set EV $REPORT_DIR/chip_top_argus_a7fp.tile_placement.rpt
set efh [open $EV w]
puts $efh "# hart x_llx y_lly orient  (expect x in {30 475 920 1365 1810 2255}, y in {565.085 1268.085 1971.085}, R0)"
set okcols {30.000 475.000 920.000 1365.000 1810.000 2255.000}
set okrows {565.085 1268.085 1971.085}
set allok 1
for {set h 0} {$h < 18} {incr h} {
	set ip [dbGet -p top.insts.name mcu0/hart$h]
	if {$ip eq "" || $ip eq "0x0"} { puts $efh "hart$h NOT_FOUND"; set allok 0; continue }
	set x [format %.3f [dbGet $ip.box_llx]]
	set y [format %.3f [dbGet $ip.box_lly]]
	set o [dbGet $ip.orient]
	set xok [expr {[lsearch -exact $okcols $x] >= 0}]
	set yok [expr {[lsearch -exact $okrows $y] >= 0}]
	set ook [expr {$o eq "R0"}]
	if {!($xok && $yok && $ook)} { set allok 0 }
	puts $efh "hart$h x=$x y=$y orient=$o  xok=$xok yok=$yok orientok=$ook"
}
puts $efh "ALL_TILES_OK=$allok"
close $efh
plog "### UNL A7FP ### : 18-tile placement evidence -> $EV (ALL_TILES_OK=$allok)"
...
if {![catch {fit}]} {
	catch { redraw ; dumpToGIF $OUTPUT_DIR/chip_top_argus_a7fp.full.gif }
	catch { zoomBox -50 350  2740 620  ; redraw ; dumpToGIF $OUTPUT_DIR/chip_top_argus_a7fp.B1.gif }
	catch { zoomBox -50 1180 2740 1340 ; redraw ; dumpToGIF $OUTPUT_DIR/chip_top_argus_a7fp.bandA.gif }
	catch { zoomBox -50 1880 2740 2040 ; redraw ; dumpToGIF $OUTPUT_DIR/chip_top_argus_a7fp.bandB.gif }
	catch { fit ; redraw }
...
toc
printStatus "chip_top_argus_a7fp PG-gate trial complete"
exit
