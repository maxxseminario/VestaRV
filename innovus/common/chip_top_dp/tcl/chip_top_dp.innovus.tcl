...
# hart_tile block; timing from the per-corner ETMs ../hart_tile/out/hart_tile.etm_{ss,ff}.
################################################################################

source ../shared/constants.tcl
source ../shared/procedures.tcl

set DESIGN_NAME chip_top_dp
set BASENAME    chip_top_dp
...
# it instantiates.
################################################################################
set init_verilog             "$INPUT_DIR/chip_top_dp.v ../MCU_DP/in/MCU_DP_hier.pnr.v"
...
source $SCRIPT_DIR/chip_top_dp_padlists.tcl
...
# irq_gf3 (sources 96-127; GPIO4/5 pushed the source list past 96): the DP
# netlist HAS it (C0's MCU_MP netlist predates it — unconditional here, like
# MCU_DP.innovus.tcl). 1041 centers the 31 um macro on the 1050-1064 stripe
# pair; the jogging sroute backstop already extends to 1085 in this script.
placeInstance mcu0/irq_gf3 1041 470 R0
addHaloToBlock 4 4 4 4 mcu0/irq_gf3
cutRow
...
# harden's ../hart_tile/out/hart_tile.{xsim.v,sdf} at gate-sim time). The 8lm tphn pad GDS
...
toc
printStatus "chip_top_dp (CastaliaDP G2) connected chip-top complete"
exit
