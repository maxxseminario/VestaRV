################################################################################
# Innovus script -- MCU_WOUND (wound-patch SoC) TOP-LEVEL ASSEMBLY (M14 physical flow; M16 rework)
#
# CLONED from tcl/MCU_DP.innovus.tcl (Stage G1, 2026-07-23 -- the proven
# CastaliaDP block P&R). Floorplan/PG/CTS/route recipe is BYTE-IDENTICAL; only
# the netlist/mmmc/BASENAME bindings, the TRNG ring-oscillator preserve gate
# and the pre-saveDesign count guards differ. Do not "improve" the physical
# recipe here -- fix it in MCU_DP first and re-clone.
#
# PREREQUISITE (does NOT exist yet as of 2026-07-26): this script consumes a
# HIERARCHICAL wound synthesis, in/MCU_WOUND_hier.pnr.v, derived from
# genus/out/MCU_WOUND_hier.genus.{v,sdc}. The 2026-07-26 wound cut on disk
# (genus/out/MCU_WOUND.genus.v, 120,957 cells) is the FLAT MCU_MP-style run:
# it has NO `hart_tile` module and NO hart_tile instances (the four harts are
# inlined as uniquified hart_tile_PC_RST_VAL0_... modules), and its SDC puts
# create_clock on TILE-INTERNAL pins (hart<h>/core/clk_cpu) that do not exist
# behind an ETM. A `genus/tcl/MCU_WOUND_hier.genus.tcl` (clone of
# MCU_DP_hier.genus.tcl with the wound HDL/constraint deltas: WOUND_HDL,
# BASENAME MCU_WOUND_hier, hart_tile blackboxed, no clk_cpu<h> clocks, no
# hart<h>/ram0 PGEN false paths, hart0 flash_clk_mem generated clock, the
# TRNG u_ro dont_touch block kept) must be run FIRST, then
# ./prep_top_netlist_wound.sh.
#
...
# notch) -- physical footprint from ../hart_tile/out/hart_tile.lef, timing from the
# per-corner ETMs ../hart_tile/out/hart_tile.etm_{ss,ff}.lib. Control plane (arbiter/
...
#
# Netlist: in/MCU_WOUND_hier.pnr.v = out-of-genus MCU_WOUND_hier.genus.v with
# any empty `module hart_tile` blackbox stub STRIPPED (prep_top_netlist_wound.sh)
# so innovus binds hart_tile to the LEF macro, not to an empty module.
#
# SCALE vs MCU_DP (why the frame is UNCHANGED): the tile row, SRAM row, ROM and
# analog placements are identical because the tile/macro set is identical (4x
# hart_tile, 5x sram1p16k, 1x rom_hvt_pg, por, 2x dco, 4x GlitchFilter -- the
# wound blackbox census is exactly dco0/irq_gf0/por, same three analog masters
# as DP). What grows is the CONTROL-PLANE std-cell load that has to fit in the
# same placeable band: 61,642 non-tile cells / 234,774 um2 of std cell here vs
# 25,490 / 110,684 um2 in DP (2.42x the instances, 2.12x the area), driven by
# nfc0 (87.5k um2 alone), i3c0, i2ct0, ow0, trng0, evfab0 and the 4-channel
# dma0/larger npu0. The DP placeable region (the ~212 um full-width band
# between the RAM row and the tile bottoms, plus the two RAM-row flanks and the
# strip under it) is ~0.99 mm2, so utilization goes ~11% -> ~24%. That still
# clears comfortably, and keeping 2689 x 1700 preserves the chip_top_dp/
# chip_top ring math (INT_SPAN 2690) downstream. WATCH: nfc0 is ~37% of the
# control plane and the band is only ~212 um tall -- if place_opt/route
# congestion bites, grow the band by lowering the SRAM row or the tile row, not
# by changing the die.
#
...
# DESIGN_NAME is the netlist top cell (still plain "MCU" in the wound run --
# confirmed: genus/out/MCU_WOUND.genus.v ends in `module MCU(...)`). BASENAME is
# the FLOW-unique prefix: every explicit report/output below is named by it, so
# the MCU_MP / MCU_DP / MCU_ARGUS assemblies (all design cell "MCU") cannot
# clobber this run's files. The IMPLICIT optDesign power reports (MCU_postCTS.power
# / MCU_postRoute.power) are still named by DESIGN cell and WILL collide across
# the MCU-named flows -- that is exactly why the explicit ${BASENAME}_*_full.power
# pair below exists (gen_power_dashboard.py reads those).
set DESIGN_NAME MCU
set BASENAME    MCU_WOUND
...
# Design import
################################################################################
set init_verilog             "$INPUT_DIR/MCU_WOUND_hier.pnr.v"
...
# ../hart_tile/out/hart_tile.etm_{ss,ff}.lib) -- the tile is a timed macro exactly like
...
# irq_gf3 (sources 96-127) exists in EVERY config since GPIO4/5 pushed the
# source list past 96 (D1/D3); the M19c-era script predates it and nanoroute
# NRIG-92'd the auto-placed macro off-grid (Stage F2a first DP assembly run).
# 1041 centers its 31 um span (1041-1072) on the 1050-1064 stripe pair --
# same +9 offset idiom as the gf2 fix above. The jogging sroute backstop
# area below is extended to cover it.
placeInstance irq_gf3 1041 470 R0
addHaloToBlock 4 4 4 4 irq_gf3
cutRow
...
sroute \
	...
	-area { 440 460 1085 515 }

# --- WOUND-ONLY: PG-fabric baseline for the pre-saveDesign count guards.
# CLAUDE.md's editDelete class (a bare `editDelete` wipes EVERY wire and
# special wire in the design, and dbDeleteObj returns rc=0 while cutting via
# structs): this script DOES delete special shapes (the ring-detour block
# above), so record the size of the PG fabric now, right after it is complete
# and before any routing exists, and gate on it before every saveDesign.
# NB `dbGet ... -e` returns the literal 0x0 on an empty result -- llength of
# that is 1, so it is normalized here.
set __sw [dbGet top.nets.sWires -e]
set PG_SWIRES_BASE [expr {($__sw eq "0x0" || $__sw eq "") ? 0 : [llength $__sw]}]
set __in [dbGet top.insts.name -e]
set PG_INSTS_BASE  [expr {($__in eq "0x0" || $__in eq "") ? 0 : [llength $__in]}]
...
# WOUND NOTE: `g11710` does NOT exist in the 2026-07-26 flat wound netlist
# (grep: 0 hits; it is a genus-mapped inverter name from the DP hier cut), so
# BOTH entries will almost certainly print "gating-check disable SKIPPED" on
# the first wound run. Kept verbatim rather than guessed: after the first
# postRoute hold signoff, read the violating ClockMuxGlitchFree gating endpoint
# out of $BASENAME.report_timing.hold.signoff.rpt and replace this list.
...
# --- WOUND-ONLY: TRNG0 ring-oscillator ensemble preserve gate -------------
# The wound SoC carries a REAL ring-oscillator ensemble, `u_ro`
# (TrngRoEnsemble, sibling of trng0): 8 rings of prime stage counts
# {13,17,19,23,29,31,37,41} = 202 preserved inverters + 8 NAND enables = 210
# cells that are DELIBERATE COMBINATIONAL LOOPS. MCU_DP has no such structure;
# this block has no counterpart in the golden script.
#
# genus/out/MCU_WOUND*.genus.sdc already emits `set_dont_touch [get_cells u_ro]`
# plus one line per ring cell, and the mmmc constraint_mode reads it -- but SDC
# dont_touch is a TIMING-side attribute; the DB attribute is what stops
# place_opt/ccopt/optDesign from resizing, cloning or deleting a ring stage
# (a collapsed ring = a silent no-entropy TRNG, the exact failure the genus
# flow FATALs on). Set it on the DB the hart_tile.innovus.tcl way -- `dbSet
# <inst>.dontTouch true`, NOT setAttribute (-dont_touch is not an option in
# 20.12, IMPTCM-48) -- and census. FATAL only on ZERO matches (pattern wrong /
# rings gone); a short count is a loud warning so a first bring-up run is not
# aborted on a naming assumption.
set RO_INSTS [dbGet -p top.insts.name u_ro/* -e]
set RO_N 0
foreach __ri $RO_INSTS {
	dbSet $__ri.dontTouch true
	incr RO_N
}
puts "### UNL STATUS ### : TRNG RO preserve -- dontTouch set on $RO_N u_ro/* instances (want >= 210)"
if {$RO_N == 0} {
	puts "FATAL (TRNG RO): no u_ro/* instances found -- the ring ensemble is absent or renamed."
	exit 1
}
if {$RO_N < 210} {
...
# so the Argus assembly flow (also design MCU) cannot clobber it.
...
# --- WOUND-ONLY: global count guards. Nothing may be written to disk on a
# gutted DB. `top.wires` is NOT a dbGet path -- signal wires live under
# top.nets.wires (same for sWires); getting that wrong is how a corpse passes
# for a design. Also re-census the TRNG rings: opt/CTS/route ran since the
# dontTouch was set, and a lost ring stage is silent.
proc __wound_count {path} {
	set r [dbGet $path -e]
	if {$r eq "0x0" || $r eq ""} { return 0 }
	return [llength $r]
}
set G_INSTS  [__wound_count top.insts.name]
set G_SWIRES [__wound_count top.nets.sWires]
set G_WIRES  [__wound_count top.nets.wires]
set __ro [dbGet -p top.insts.name u_ro/* -e]
set G_RO [expr {($__ro eq "0x0" || $__ro eq "") ? 0 : [llength $__ro]}]
puts "### UNL STATUS ### : signoff census -- insts $G_INSTS (base $PG_INSTS_BASE), sWires $G_SWIRES (base $PG_SWIRES_BASE), wires $G_WIRES, u_ro cells $G_RO"
if {$G_INSTS == 0 || $G_SWIRES == 0 || $G_WIRES == 0} {
	puts "FATAL (count guard): insts/sWires/wires collapsed -- refusing to saveDesign a gutted database."
	exit 1
}
if {$G_SWIRES < [expr {$PG_SWIRES_BASE / 2}]} {
	puts "FATAL (count guard): special-wire count $G_SWIRES is less than half the post-power baseline $PG_SWIRES_BASE -- PG fabric was destroyed."
	exit 1
}
if {$G_RO < 210} {
	puts "WARNING (TRNG RO): signoff census $G_RO ring cells (< 210) -- entropy path may have been optimized."
}
...
# Second count guard: addFiller / streamOut / saveNetlist ran since the first
# one -- same class, same bar, before the final DB hits disk.
set F_INSTS  [__wound_count top.insts.name]
set F_SWIRES [__wound_count top.nets.sWires]
set F_WIRES  [__wound_count top.nets.wires]
puts "### UNL STATUS ### : final census -- insts $F_INSTS, sWires $F_SWIRES, wires $F_WIRES"
if {$F_INSTS == 0 || $F_SWIRES == 0 || $F_WIRES == 0} {
	puts "FATAL (count guard): insts/sWires/wires collapsed before the final saveDesign."
	exit 1
}
if {$F_SWIRES < [expr {$PG_SWIRES_BASE / 2}]} {
	puts "FATAL (count guard): final special-wire count $F_SWIRES below half the post-power baseline $PG_SWIRES_BASE."
	exit 1
}

saveDesign $DATABASE_DIR/$BASENAME.final.innovus -def -netlist -rc -tcon
toc
printStatus "MCU_WOUND top assembly complete"
exit
