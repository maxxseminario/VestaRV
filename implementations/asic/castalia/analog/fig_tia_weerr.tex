...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}
    ...
    \end{axis}
  \end{tikzpicture}}
  \caption[{Error between the working-electrode potential and $v_{\mathrm{cm}}$ across the same sweep.}]{Error between the working-electrode potential and $v_{\mathrm{cm}}$ across the same sweep. It stays inside \SI{\pm 0.4}{\milli\volt} in both directions; the slope is the stage's input impedance, set by the loop gain at the summing node. That is under \SI{0.04}{\percent} of a \SI{1.25}{\volt} full scale, so the virtual ground is not an accuracy limit at this gain.}
  \label{fig:tia-weerr}
\end{figure}
