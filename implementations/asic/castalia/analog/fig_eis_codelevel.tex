%% VestaRV: hand-written TRM fragment, not maestro2tex output (one transient record
...
%%
%% Nothing inputs this file: tab_eis_codelevel.tex carries every value below, plus the
%% two things drawn here spatially (the +-0.573 deg gate as a column, the -180 f/f_s
%% offset the interleaved reads sit on as a row) and the measured / corrected /
%% analytic distinction this figure makes with filled and open markers.
%% \label{fig:eis-codelevel} moved with the content and is now
%% \label{tab:eis-codelevel}. Restoring the figure is putting the \input back in
%% note_eis_accuracy.tex, moving the label back, and turning that fragment's Table~\ref
%% into a Figure~\ref.
%%
...
%%
%% Caveats that must survive into the caption: both panels are referred to the ac
