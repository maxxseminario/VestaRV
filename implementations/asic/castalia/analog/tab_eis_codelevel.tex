...
%%
%% SET INLINE, NOT AS A FLOAT (2026-08-27).  The section's prose ends part-way
%% down its last page and the \FloatBarrier that closes it turned a deferred
%% float into a \newpage (placeins), so a 12-row table was costing a whole page
%% of its own.  Written as an ordinary vertical box in the text stream instead:
%% the table counter, the \label and the short-form List of Tables entry are
%% unchanged because \captionof{table} steps and writes exactly what \caption
%% does inside a table environment.
%% NOT \begin{table}[H]: the float package, which defines the H specifier, is
%% commented out in platform/common/latex/packages-commands.template.tex (that
%% preamble is shared across chips and is not ours to add to).  caption IS
%% loaded there, so \captionof is the available route.
%% The box must not break across a page -- caption on one page and body on the
%% next is worse than the float was -- hence the \parbox, with the \addvspace
%% pair standing in for the float's \intextsep.
%% THE \parbox IS LOAD-BEARING, not cosmetic.  Written as loose vertical
%% material the block was unbreakable anyway, but \captionof's
%% \addcontentsline whatsit is NOT: the page builder flushed it to the page
%% BEFORE the one the table landed on, and the List of Tables then printed the
%% wrong page number (114 for a table on 115) while \ref and the hyperlink were
%% both right -- a wrong number that no lint and no log line reports.  Inside a
%% box the whatsit ships with the box.  Verified in the built TRM.lot.
%% [b] so the box is all height and no depth: a 320 pt depth on the line that
%% carries it is what \maxdepth exists to police.  It renders identically to
%% the default alignment here (byte-identical PDF) -- it is insurance against a
%% future reflow that lands the block elsewhere on a page.
%%
%% 2026-08-27, CAPTION CUT (nine rendered lines / 122 pt -> six).  Four
%% sentences before and after; the sentences were the long thing, not the
%% count.  WHAT LEFT, and where it still lives:
%%   - "the excitation synthesised by the real 12-bit ladder": the section's
%%     own prose already says the record was "excited by the real reference
%%     ladder and read through the input multiplexer and an extracted
%%     converter" (note_eis_accuracy.tex), so the condition is stated once in
%%     the section instead of twice.  Provenance for it is unchanged above:
%%     code pair 2032/2064 out of the real ladder.
%%   - "three successive readings of that one record" -> "magnitude rows
%%     accumulate to the total", which is the part a reader must have to read
%%     the block (and which stops the phase rows being summed the same way).
%%     The count of three is the three rows themselves.
%%   - "64 conversions per excitation period": ARITHMETIC ON TWO NUMBERS THAT
%%     ARE BOTH STILL IN THE CAPTION -- 869 565 / 13 587 = 64.0 exactly, which
%%     is also what makes the -180 f/f_s row 2.8125 deg.  Dropped as derived,
%%     not as a measured quantity; it is still in the provenance block above.
%%   - "one record of" in the opening sentence, because the closing sentence
%%     says "One record, one die, one frequency" in bold and the caveat is
%%     stated once and tersely rather than twice.
%% NOTHING NUMERIC LEFT.  The two caveats that change how a number reads are
%% both still here and still attached to their number: the reference for BOTH
%% blocks is the small-signal analysis, itself +0.295 % and 0.405 deg from the
%% analytic load (without which the +0.343 % total reads as an absolute error
%% and is not one); and ONE record, ONE die, ONE frequency into a RESISTIVE
%% load with no noise sources -- a deterministic attribution, nothing to quote
%% a sigma from.  Archive for everything in and around this table:
%% ~/chips/castalia/simruns/eisfigs2_20260826 (data/fig5a_attribution.csv,
%% data/fig5b_phase.csv) and ~/chips/castalia/simruns/eisdeep_20260826
%% (code_report/).
\par\addvspace{\intextsep}
\noindent\parbox[b]{\textwidth}{%
  \captionof{table}[{Impedance from converter codes: magnitude attribution and the phase
  mechanism.}]{Impedance demodulated from the ten-bit words of the extracted
  converter: \num{256} conversions at \SI{869565}{\per\second}, the
  multiplexer interleaving the transimpedance output and the reference
  electrode, excitation \SI{13.587}{\kilo\hertz} into \SI{11.000}{\kilo\ohm} at
  gain code 20. Magnitude rows accumulate to the total; phase rows are
  independent, and \(-180\,f/f_s\) is the analytic half-conversion grid offset
  the two interleaved ones sit on. Both are referred to the small-signal
  analysis of the same load and gain code, itself \(+0.295\,\%\) and
  \SI{0.405}{\degree} from the analytic load. \textbf{One record, one die, one
  frequency into a resistive load}, no noise sources in the deck: a
