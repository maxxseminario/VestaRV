...
%%
%% This table replaces the figure fig_eis_codelevel.tex, which is on disk and no longer
%% input; restoring it is putting an \input back and moving the \label. Every plotted
%% value is a row here, at the precision of the source data rather than of a bar top.
%% What the figure carried spatially, this table carries otherwise:
%%   - filled vs open markers (measured vs corrected vs analytic control) -> the
%%     Reading column.
%%   - the +-0.573 deg gate band -> the gate column.
%%   - the dashed -180 f/f_s line the two interleaved reads sit on -> a row of its own,
%%     adjacent to them, marked analytic.
%%
...
%%
%% Caveats that must survive into the caption: both blocks are referred to the ac
%% analysis of the same load and gain code, whose own error against the analytic load
%% is +0.295 % and +0.405 deg (against the analytic load the codes read +0.639 % and
%% -2.533 deg); one record, one die, one frequency, one resistive load, no noise
%% sources in the deck -- a deterministic attribution
...
%%
%% Set inline, not as a float. The section's prose ends part-way down its last page and
%% the \FloatBarrier that closes it turns a deferred float into a \newpage (placeins),
%% so a 12-row table costs a whole page. As an ordinary vertical box in the text stream
%% the table counter, the \label and the short-form List of Tables entry are unchanged,
%% because \captionof{table} steps and writes exactly what \caption does inside a table
%% environment. Not \begin{table}[H]: the float package, which defines the H specifier,
%% is commented out in platform/common/latex/packages-commands.template.tex, a shared
%% preamble that is not ours to add to; caption is loaded there, so \captionof is the
%% available route.
%% The \parbox is load-bearing. The box must not break across a page, and although
%% loose vertical material would be unbreakable anyway, \captionof's \addcontentsline
%% whatsit is not: the page builder flushes it to the page before the one the table
%% lands on, and the List of Tables then prints a page number one short while \ref and
%% the hyperlink are both right -- an error no lint and no log line reports. Inside a
%% box the whatsit ships with the box. The \addvspace pair stands in for the float's
%% \intextsep, and [b] makes the box all height and no depth, which is what \maxdepth
%% exists to police; it renders identically to the default alignment here and is
%% insurance against a future reflow.
%%
%% Two caveats in the caption change how a number reads and must stay attached to it:
%% the reference for both blocks is the small-signal analysis, itself +0.295 % and
%% 0.405 deg from the analytic load (without which the +0.343 % total reads as an
%% absolute error and is not one); and it is one record, one die, one frequency into a
%% resistive load with no noise sources, a deterministic attribution with no sigma to
%% quote. Archive: ~/chips/castalia/simruns/eisfigs2_20260826
%% (data/fig5a_attribution.csv, data/fig5b_phase.csv) and
%% ~/chips/castalia/simruns/eisdeep_20260826 (code_report/).
