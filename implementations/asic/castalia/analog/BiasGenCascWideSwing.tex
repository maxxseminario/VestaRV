...
% 2026-08-25: tab_biasgen_dcop.tex is SUPERSEDED and removed from this input
% list -- its corner definitions pinned the degeneration resistor at typical,
% which made the I_DD corner spread read 0.30 uA instead of 17.6 uA.  It could
% not be regenerated (the corrected run has no maestro2tex config), so the
% replacement is the hand-written tab_biasgen_dcop_free.tex, which carries the
% same table with the resistor freed.  The generated file is left on disk,
% unreferenced.  NOTE for whoever regenerates this master: this one line will
% come back; make the same swap, or teach the config the new run.
