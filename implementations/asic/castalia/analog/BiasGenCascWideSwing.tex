...
parasitics) of the \texttt{BiasGen\allowbreak Casc\allowbreak Wide\allowbreak Swing\allowbreak\_tb} testbench, over the process,
...
% The DC operating point comes from the HAND-WRITTEN tab_biasgen_dcop_free.tex,
% not from this bench.  The generated tab_biasgen_dcop was superseded on
% 2026-08-25: the imported tsmcN65_typ_* corners carry cor_res.scs
% section="tt_res" in EVERY corner, pinning the poly resistor that sets the
% reference current, so its I_DD corner spread read 0.30 uA against a true
% 18.375 (ss) - 35.965 (ff) uA = 17.6 uA.  The corrected numbers come from
% castalia/tb_anatop_bg_startup (simruns/bgladder_20260825/tables/
% bg_startup_matrix.tsv, rows r1u_*), a bench this config cannot read.  See
% the `superseded` key: regenerating the old table is now a hard error.
\input{\MaestroRoot tab_biasgen_dcop_free.tex}
