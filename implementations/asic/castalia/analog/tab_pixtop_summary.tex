...
%%
%% 2026-08-24: two Monte Carlo rows ADDED at the foot -- sigma 5.754 mV and
%% 5.552 mV, from simruns/pixmc_20260823/mc_distributions.txt (200 samples,
%% process + mismatch, section castalia_pm_25, nominal corner, 37 C).  A third
%% row, "Per-channel yield, calibrated 61.5 % / 0.50 % uncalibrated", was
%% written and then REMOVED the same day on the owner's instruction "do not
%% talk about pixel level yield yet"; yield_table.txt still holds it.  The
%% dc rows above are UNCHANGED: they are the typical point, and the nominal run
%% of the Monte Carlo deck reproduces them to the printed digits.  The tracking
%% rows' conditions cells now say which of the two ranges is the specification
%% (the 0.4-2.1 V window, owner decision 2026-08-24, sss:pixtop-mc).
\begin{table}[!htb]
  \centering
  ...
  channel staying at schematic level; rows 15--18 are the post-fix
  small-signal, noise and power runs of
  Sections~\ref{sss:pixtop-stability}--\ref{sss:pixtop-power}; the final two
