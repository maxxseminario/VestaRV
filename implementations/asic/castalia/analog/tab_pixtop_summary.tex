...
%%
%% 2026-08-24: three Monte Carlo rows ADDED at the foot -- sigma 5.754 mV and
%% 5.552 mV and the 61.5 % / 0.50 % yields, from
%% simruns/pixmc_20260823/{mc_distributions,yield_table}.txt (200 samples,
%% process + mismatch, section castalia_pm_25, nominal corner, 37 C).  The
...
\begin{table}[!htb]
  ...
  are Monte Carlo, and their yield is an upper bound for the reason
  ...
  \begin{tabular}{@{}%
    ...
    Reference tracking error, \(\sigma\) &
      ...
      (\S\ref{sss:pixel-mc}) \\
    Per-channel yield, calibrated &
      \SI{61.5}{\percent} &
      same run, all 20 specifications, tracking over the specified window;
      \SI{0.50}{\percent} uncalibrated. Upper bound \\
    \bottomrule
  \end{tabular}}
\end{table}
