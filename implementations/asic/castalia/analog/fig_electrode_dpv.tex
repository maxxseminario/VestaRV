%% VestaRV: hand-written TRM fragment, not maestro2tex output (XY figure kind).
%%
...
\begin{figure}[htbp]
  ...
  pulses at a \SI{200}{\milli\second} period, every level quantised to
  \SI{152.6}{\micro\volt}), with the two current-sampling instants
  \SI{5}{\milli\second} before each edge. Bottom: the differential current over the full
  ...
  Figure~\ref{fig:electrode-panel} in readout, gain tap and reporter set. The
  \SI{152.6}{\micro\volt} step is finer than the \SI{610.35}{\micro\volt} LSB of
  the shipped 12-bit reference DAC (Section~\ref{ss:dacr2r12}), so the apex
  accuracy read off this panel is a lower bound on the channel's.}
