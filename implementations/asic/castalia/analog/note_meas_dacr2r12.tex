...
%
% UPDATED 2026-09-06: steps 3 and 4 said to fit the reference droop and to read
% the supply node as delivered.  The two per-pixel supply regulators were deleted
% from anatop_pixel on 2026-08-24, so both are now conditional on a regulator
% being reinstated.  Nothing else in the procedure changes: the code sweep is
% still where a broken ladder shows.
...
\item Sweep all 4096 codes into the converter with
  ...
  pattern as delivered, Section~\ref{sss:adc-power}). The supply regulators
  were deleted from the cell (Section~\ref{sss:dacr2r12-supply}), so the ladder
  reference is the \SI{2.5}{\volt} analog rail and the one-parameter droop fit
  applies only if a regulator is reinstated. Do not store the per-code
  ...
  ladder shows.
\item Chip-level slot 7 reads the DAC supply node directly, which is the only
  place a reinstated regulator's \SI{25.8}{\milli\volt} code-dependent
  excursion could be confirmed or refuted on silicon. Chip-level slot 6 reads
  the shared common-mode DAC, forceable
