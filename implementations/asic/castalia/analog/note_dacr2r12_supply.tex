...
%
% RETITLED 2026-09-06 as the removed-block subsection, and moved behind the Monte
% Carlo subsection by the same `order` edit.  The supply block anatop_pixel_dac_psu
% was deleted from anatop_pixel on 2026-08-24 (firmware contract 1.4; 806.8 uW
% returned), so every figure and table placed after this fragment characterises a
% path the tapeout channel does not have.  Nothing was deleted and no \label was
% changed: the fragments carry live \ref targets from note_dacr2r12_scope,
% fig_dacr2r12_dnl_ratio, fig_dacr2r12_mc_loadreg, fig_dacr2r12_vddac,
% fig_dacr2r12_inl_wpsu, tab_dacr2r12_loadreg and fig_dacr2r12_psu_load.
%
...
\subsubsection{Removed block: the DAC supply regulator}
\label{sss:dacr2r12-supply}

\textbf{The two per-pixel DAC supply regulators were deleted from
\texttt{anatop\_pixel} on 2026-08-24, returning \SI{806.8}{\micro\watt}, and both
ladders now run from the \SI{2.5}{\volt} analog rail directly.} Everything in this
subsection was measured through that block and describes a path the tapeout channel
does not have, so the \SI{25.8}{\milli\volt} reference movement below is no longer
modelled rather than smaller, and the \SI{528.6}{\micro\volt} LSB it implies does not
apply to the shipping cell.
