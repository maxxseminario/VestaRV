...
\SI{400}{\micro\ampere} rail budget by \(2.3\times\). That range is a lower
bound, and by a wide factor: the rail is dominated by bias-derived branches, the
reference resistor that sets them is poly, and the corner set holds the passive
model rows at typical (pinned-passive caveat,
page~\pageref{p:pinned-passives}). Freeing them widens the amplifier bench's own
supply box by \(4.2\times\).
...
keep the passive model rows at typical (page~\pageref{p:pinned-passives}), and
for a block whose accuracy is set by matching they exercise almost nothing.
Monte Carlo is the real test, and
