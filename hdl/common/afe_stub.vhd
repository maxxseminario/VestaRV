-- VestaRV: AFE register stub
-- Digital register stub for one analog front-end site or for the shared EIS engine: 16 words in one 64 B sub-slot, standing in for the analog IP. Only addr(3:0) decodes, so the block aliases every 16 words within its sub-slot.
-- Every access is qualified against the arbiter's granted-master index: an AFE for hart h answers only when s_master is h or MGMT_HART, and EIS is instantiated with OWNER_HART equal to MGMT_HART. A denied read returns 0 and a denied write is dropped: no bus error, no stall, no arbiter-contract change, and no way to forge ownership.
...
-- Words +0x0 CTRL, +0x1 DACPAT, +0x2 TIA, +0x3 SWM, +0x4 ADCC, +0x5 ADCD, +0x6 STAT, +0x7 IF (W1C, a set bit drives irq), +0x8..0xF scratch. A CTRL write with wdata bit 0 = 1 soft-sets IF bit 0, the test hook that exercises the interrupt path until the analog IP drives real events.
