-- VestaRV: Debug Module
-- One per chip at assembly level, always-on mclk domain, never inside a tile: DMI register file, per-hart run control, abstract access-register commands, a 2-word program buffer with an implicit third word, and halt groups.
...
-- No DTM/TAP, no triggers, no System Bus Access: sbcs reads all zeros, 0x39-0x3F answer failed, and memory access is progbuf lw/sw through the hart.
