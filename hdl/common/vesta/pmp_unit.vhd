-- VestaRV: RISC-V physical memory protection (Smpmp) match, priority and permission decode
...
-- Every access is at most 4 bytes and never crosses a word boundary, and every region boundary is word-aligned, so an access matches iff its word address (bits 31:2) lies in the region. Needed permissions come from the caller: fetch needs X, load needs R, store needs W, and LR/SC/AMO needs R and W together.
