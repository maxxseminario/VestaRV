...
-------------------------------------------------------------------------------
-- NPU_xnor_tb.vhd: XNOR/popcount mode (NPUCR.MODE=2) bench, npu_xnor_design.md D9.
-- A SEPARATE leg from NPU_conv_tb, per D9/D10-S2.
--
-- Instantiates the SAME NPU entity plus staging-RAM model at the MCU/chip generics (X_M=0, W_M=7, Y_M=7, N=24, RHO=2) and drives it through golden-file-driven cases.
-- D9/GX4: the +/-1.0 output words are only exact at these generics.
-- Golden files (npu_xnor_<case>_{cfg,in,w,exp}.txt) are generated by verification/npu/gen_xnor_vectors.py and reach this run directory as symlinks, the same mechanism as the npu_conv_*/npu_fp_* files.
--
-- cfg header (FROZEN order, D9/D5): K N THRESH IVSAR WVSAR OVSAR.
-- NW = ceil(K/32) is DERIVED here identically to the golden model (D9 Q4); it is not a header field.
--
-- in/w files hold arbitrary 32-bit PACKED BIT PATTERNS, not fixed-point values.
-- Unlike the conv bench's in/w/exp files, these routinely exceed the signed-32-bit range representable by a plain VHDL INTEGER literal (e.g. 0xFF000000 = 4278190080).
-- A custom decimal-string parser (parse_u32) reads them as raw bit patterns via a wide accumulator, avoiding a `std.textio.read`-into-INTEGER overflow.
-- The cfg and exp files only ever hold small values (K/N/THRESH/addresses, within +/-16777216) and are read with the plain `read` procedure like the conv bench.
--
...
--                The ALL-PASS banner prints iff the running error tally is 0.
--   1:           deliberately corrupts EXACTLY ONE expected value (the first output of the base case) before comparison; every other case is untouched.
--                The NEGCTRL-PASS banner prints iff the tally is EXACTLY 1.
--
-- The base-case invocation is ALSO the D9 G-XCOLLAPSE proof: it is the very first thing this bench does after ResetN deasserts (no warm-up CFG pokes ahead of it), and every one of its outputs is additionally checked for definedness (no X/U/Z/W/- bit) before the numeric compare.
-------------------------------------------------------------------------------
...
architecture testbench of NPU_xnor_tb is
	----- Clock Information
	-- 25 MHz / 40 ns is the BENCH-DRIVEN closure rate (the digperiphs PERIOD DECISION): the genus NPU close and its SDF are at 40 ns.
	-- NOT the legacy NPU_tb's 100 MHz; this follows the NPU_conv_tb precedent.
	...
	-- Memory address bus constants, from MemoryMap.vhd.
	-- NPU_tb/NPU_conv_tb precedent: define them locally rather than pulling work.MemoryMap in here.
	...
	-- FROZEN MCU/chip generics, the npu_xnor_design.md binding inputs.
	-- D9/GX4: NPU_tb's own generic DEFAULTS are the bench config Q0.15/Q3.15, NOT these.
	-- Using the bench defaults here would diverge from silicon and from the +/-1.0 word encoding of gen_xnor_vectors.py's golden model.
	...
	-- Bounded poll ceiling: the worst predicted case (extremes_n256, D8) is about 1539 NpuClk cycles.
...
begin
	...
	SIM_PROCESS: process
		...
		-- parse_u32: converts a decimal string into a raw 32-bit std_logic_vector using a wide (64-bit) accumulator.
		-- Values up to 2**32-1 overflow a plain VHDL INTEGER, whose range tops out at 2**31-1, so this avoids a std.textio `read`-into-INTEGER range error.
		-- The in/w golden files hold arbitrary packed bit patterns rather than fixed-point values, so this is load-bearing; see the file header comment.
		-- An optional leading '-' is handled too: unused by in/w today, since gen_xnor_vectors.py's pack_bits/with_tail_garbage only ever emit non-negative python ints, but cheap to keep general.
		-- A PROCEDURE, not a function: VHDL-93/2002 (2.1.1) forbids access types (line) as function formal parameters, so only a procedure may take one, as a variable-class parameter.
		...
		begin
			for i in l_in.all'range loop
				...
				if ch = '-' then
					...
					-- unsigned "*" returns a double-width result, 128 bits here, so resize back to 64 before the "+" to match the width of acc.
					-- Lossless: every intermediate value stays far below 2**64.
		...
		-- mmr_write: one MMR register write, all transitions on the falling MMR clock edge.
		...
		-- MabSramCLK is a GATED clock: SramClkEn is driven from MabSramCEN through ClkGate, so it only toggles once CEN is already asserted.
		-- A burst of words is therefore: assert CEN, synchronized off the FREE-RUNNING Clk, which opens the gate; THEN wait on MabSramCLK edges for every word; only deassert CEN once the whole burst is done.
		...
		-- think_start: pokes IVSAR/WVSAR/OVSAR/NPUCFG1(THRESH)/NPUCFG2(K)/NPUCR for one case.
		-- NPUCR gets MODE=XNOR, THINK=1, NI=NW-1, NN=N-1, with BEN=AEN=ACTF=TDIE=0 per the D6/GX2 firmware contract.
		-- NW = ceil(K/32) is derived here (D9 Q4), identically to gen_xnor_vectors.py.
		...
		-- run_xnor_case: loads npu_xnor_<case_name>_{cfg,in,w,exp}.txt (the D9 golden-file interface), runs the THINK, and compares every output.
	...
	begin
		...
		----------------------------------------------------------------
		-- G-BASE / G-XCOLLAPSE / G-NEG: the FIRST thing this bench does after reset is run the base case, with no other CFG pokes ahead of it and every output checked for definedness.
		-- D9 G-XCOLLAPSE: that proves the added XNOR registers cannot produce X on the SRAM interface after a cold reset.
		...
		----------------------------------------------------------------
		-- NPUCFG1(THRESH)/NPUCFG2(K) readback smoke: the D5 contract is an exact full write/read.
		----------------------------------------------------------------
