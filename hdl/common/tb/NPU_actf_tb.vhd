-- VestaRV: NPU activation-mux testbench
-- ACTF bench (NPUCR.MODE = 0 MLP, ACTF = NPUCR(25:23)) at the chip generics X_M=0, W_M=7, Y_M=7, N=24, RHO=2.
-- MLP with NI=0 and BEN=0 makes each neuron a single MAC tap, acc = X_HALF * w_neuron, so the golden generator can stage any exact Q7.24 accumulator by picking w_neuron = 2*target.
-- Cases come from npu_actf_<case>_{cfg,in,w,exp}.txt, symlinked into the run directory. The cfg header order is fixed: NI NN BEN AEN ACTF IVSAR WVSAR OVSAR; in.txt is NI+1 Q0.24 words, w.txt is (NN+1)*(NI+1+BEN) Q7.24 words, exp.txt is NN+1 post-activation words (the raw accumulator when AEN=0).
