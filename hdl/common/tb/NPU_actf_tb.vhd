...
-------------------------------------------------------------------------------
-- NPU_actf_tb.vhd: ACTF activation-mux bench (NPUCR.MODE=0/MLP, NPUCR.ACTF=NPUCR(25:23)), npu_actf_design.md D10 (P4.4).
-- Clone of the NPU_gemm_tb/NPU_conv_tb skeleton, at the SAME MCU/chip generics (X_M=0, W_M=7, Y_M=7, N=24, RHO=2) and 25 MHz bench-driven closure rate.
--
-- MODE 0 (MLP) is deliberately the simplest way to stage an EXACT target accumulator (npu_actf_design.md D10 KEY FINDING).
-- Every case here uses NI=0 (ONE shared input X_HALF = 0.5 real, Q0.24) and BEN=0, so each neuron's accumulator is a single MAC tap `acc = X_HALF * w_neuron`.
-- The golden generator (verification/npu/gen_actf_vectors.py) picks `w_neuron = 2*target` so the tap lands EXACTLY on the desired signed Q7.24 accumulator with zero rounding.
-- Every boundary/knee value this bench exercises is therefore exact.
--
-- Golden files (npu_actf_<case>_{cfg,in,w,exp}.txt) are generated by gen_actf_vectors.py and reach this run directory as symlinks, the npu_conv_*/npu_gemm_*/npu_xnor_* mechanism.
--
-- cfg header (FROZEN order, D10/kickoff): NI NN BEN AEN ACTF IVSAR WVSAR OVSAR.
-- in.txt holds NI+1 raw Q0.24 signed words, always 1 here.
-- w.txt holds (NN+1)*(NI+1+BEN) raw Q7.24 signed words, walked contiguously exactly as think_layer() and the DUT's own weight pointer do.
-- exp.txt holds NN+1 raw signed words: post-activation, or the raw accumulator when AEN=0.
--
-- Generic NEGCTRL (D10 G-NEG, MANDATORY LAST):
--   0 (default): clean run; the ALL-PASS banner prints iff errors=0.
--   1:           deliberately corrupts EXACTLY ONE expected value, the MID-POSITION output ((NN+1)/2, integer division) of the FINAL case invocation.
--                That invocation is a rerun of "tanh", positioned as the absolute last action in this process per D10's explicit "G-NEG LAST" ordering requirement.
--                Every other group/case in this bench is unaffected by NEGCTRL, and the NEGCTRL-PASS banner prints iff errors=1.
--
-- The "sigregress" case invocation is the FIRST thing this bench does after ResetN deasserts, with no warm-up CFG pokes ahead of it.
-- That is the D10 G-XCOLLAPSE proof: `actf_run` and the new act_out/relu_word/tanh_word/clamp_word/exp_word nets must resolve to defined outputs from a cold, X-filled staging RAM once CFG is written.
-- Every one of its outputs is additionally checked for definedness (no X/U/Z/W/- bit) before the numeric compare.
-- The NPUCR ACTF-field readback smoke runs immediately after sigregress and before the rest of the case list, which is harmless to any case's correctness since think_start fully overwrites NPUCR every time.
-------------------------------------------------------------------------------
...
architecture testbench of NPU_actf_tb is
	...
	-- NPU_tb/NPU_conv_tb/NPU_xnor_tb/NPU_gemm_tb precedent: define them locally rather than pulling work.MemoryMap in here.
	...
	-- FROZEN MCU/chip generics, the npu_actf_design.md binding inputs.
	-- They MUST match gen_actf_vectors.py's fixed-point encoding, not the legacy NPU_tb bench-default generics.
	...
	-- Bounded poll ceiling: T = 1 + NN*(5*NI + 3*B + 1) + 2, with NI=B=0 here, so the largest case (tanh, NN=7) predicts T=10 cycles.
	-- This bound is deliberately generous.
...
begin
	...
	SIM_PROCESS: process
		...
		-- Identical to the NPU_conv_tb/NPU_xnor_tb/NPU_gemm_tb copy.
		...
		----- MMR primitives, mirroring the falling-edge idiom of NPU_tb/NPU_conv_tb/NPU_xnor_tb/NPU_gemm_tb -----
		...
		----- SRAM primitives: backdoor MCU-side access, mirroring NPU_tb/NPU_conv_tb/NPU_xnor_tb/NPU_gemm_tb -----
		...
		-- D10 golden-file interface: cfg = NI NN BEN AEN ACTF IVSAR WVSAR OVSAR.
		--   check_no_x  : also assert each output is fully defined (G-XCOLLAPSE).
		--   corrupt_mid : deliberately corrupt the MID-POSITION expected value ((NN+1)/2, integer division) for G-NEG.
		--   force_actf  : a value >= 0 overrides the cfg file's ACTF, the G-RESERVED/G-AEN0 fallback-poke idiom from the conv/gemm precedent.
	...
	begin
		...
		----------------------------------------------------------------
		-- G-SIGMOID-REGRESS / G-XCOLLAPSE: the FIRST thing this bench does after reset is run the sigregress case, with no other CFG pokes ahead of it and every output checked for definedness.
		-- D10 G-XCOLLAPSE: that proves the P4.4 `actf_run`/act_out/relu_word/tanh_word/clamp_word/exp_word nets cannot produce X on the SRAM interface after a cold reset.
		-- It is also the ACTF=0 mux-collapse compatibility anchor (D1/GA1).
		----------------------------------------------------------------
		run_actf_case("sigregress", check_no_x => true);

		----------------------------------------------------------------
		-- NPUCR ACTF-field readback smoke, run early per the kickoff: a raw poke with THINK=0 (no launch) confirms bits 25:23 store and read back exactly.
		...
		-- G-RESERVED: ACTF=5 baked into the cfg, then ACTF=7 forced.
		-- Both reserved codes MUST equal the SAME sigmoid golden (D6).
		run_actf_case("reserved");
		run_actf_case("reserved", force_actf => 7);

		-- G-AEN0: AEN=0 with ACTF swept 0 to 4.
		-- The output MUST be the raw accumulator (passthrough) for every ACTF, per the D1 master enable.
		...
		----------------------------------------------------------------
		-- G-NEG (MANDATORY LAST, D10): the absolute final action in this process, a rerun of "tanh" that corrupts its mid-position expected output exactly once when NEGCTRL=1.
		-- Every group above is unaffected.
		----------------------------------------------------------------
