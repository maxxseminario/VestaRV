-- VestaRV: SPI pad-level BFM
-- Drives the SPI peripheral's slave port as an external master: the model drives SCK and MOSI, and the DUT in slave mode shifts out on MISO. The record members map onto the DUT's slave inputs in the TB (sck_in <= e.sck, mosi_in <= e.mosi); the TB observes miso_out and `half` is the SCK half-period.
...
-- Master-mode transfers are driven by writing the DUT's registers instead, so the DUT generates SCK and MOSI itself and any MOSI-to-MISO loopback stays a concurrent assignment in the TB.
