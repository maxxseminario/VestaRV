-- VestaRV: NPU CONV1D testbench
-- CONV1D mode (NPUCR.MODE = 1), a separate leg from the mode-0 MLP fixture NPU_tb.
...
-- Golden files npu_conv_<case>_{cfg,in,w,exp}.txt come from verification/npu/gen_conv_vectors.py and reach the run directory as symlinks.
-- The base case runs first after ResetN deasserts, with no warm-up CFG pokes ahead of it, and every output is checked for definedness before the numeric compare.
...
architecture testbench of NPU_conv_tb is
	----- Clock Information
	-- 25 MHz / 40 ns is the bench-driven closure rate: the NPU synthesis close and its SDF are at 40 ns, so the tb must drive the rate the netlist was closed at.
	...
	-- FROZEN MCU/chip generics: the NPU entity's own defaults are the bench config Q0.15/Q3.15, and using those here would diverge from silicon and from gen_conv_vectors.py's golden model.
	...
	-- Bounded poll ceiling: the largest case (multi) is ~10.3k NpuClk cycles, so this clears every case with margin while staying far short of the 100 ms / 2.4M-cycle tb watchdog.
...
begin
    ...
    -- NPU instantiation (without internal SRAM), at the MCU/chip generics.
	...
	SIM_PROCESS: process
		...
		-- has_x: definedness check, hand-written with no Is_X dependency so it compiles cleanly under -V200X.
		...
		-- calc_L: the "exact valid conv, no padding" input length; L is not stored in the cfg file, so it is derived here exactly as gen_conv_vectors.py does.
		function calc_L(k, s, d, lout : integer) return integer is
		begin
			return d * (k - 1) + 1 + (lout - 1) * s;
		end function;

		----- MMR primitives (falling-edge idiom) -----
		...
		-- think_poll: bounded poll of NPUCR bit 16, also sampling the DUT's own NpuSramGWEN_out each cycle for the Lout=0 "zero writes" check.
		...
		begin
			...
			if lout_v = 0 then
				-- Lout=0: zero outputs, immediate done, NO staging-RAM writes.
	...
	begin
		...
		-- The aen case runs twice: ACTF=0 (sigmoid), then a fallback poke with ACTF=1 (live ReLU) over the SAME cfg/in/w data.
		-- Its accumulators are mixed-sign, so ReLU differs from sigmoid on every negative one and the re-run compares against the aen_relu golden, not the case's own sigmoid exp file.
		run_conv_case("aen");
		...
		----- Final verdict: the banner demanded depends on NEGCTRL -----
		wait for 1 us;
