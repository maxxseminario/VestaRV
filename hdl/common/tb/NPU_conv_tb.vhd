...
--
-- A SEPARATE leg from NPU_tb (the mode-0 MLP compatibility fixture, left
-- untouched): instantiates the SAME NPU entity + staging-RAM model at the
-- MCU/chip generics (X_M=0, W_M=7, Y_M=7, N=24, RHO=2 -- NOT NPU_tb's bench
-- defaults, D9/G7 gotcha) and drives it through golden-file-driven GROUPs.
-- Golden files (npu_conv_<case>_{cfg,in,w,exp}.txt) are generated by
-- verification/npu/gen_conv_vectors.py and reach this run directory as
-- symlinks (same mechanism as npu_fp_*.txt for NPU_tb).
--
...
--                  first output of the G-BASE case) before comparison;
--                  every other case is untouched. NEGCTRL-PASS banner
--                  printed iff the tally is EXACTLY 1.
--
-- The G-BASE invocation is ALSO the D9 G-DUP4 "X-collapse" proof: it is the
-- very first thing this bench does after ResetN deasserts (no warm-up CFG
-- pokes ahead of it), and every one of its outputs is additionally checked
-- for definedness (no X/U/Z/W/- bit) before the numeric compare.
-------------------------------------------------------------------------------
...
architecture testbench of NPU_conv_tb is
	...
	-- DECISION -- the genus NPU close and its SDF are at 40 ns, and the tb
	-- must drive the rate the netlist was closed at; the legacy NPU_tb's
	-- 100 MHz was cloned here by mistake and caught at SDF bring-up).
	...
	-- Memory Address Bus Constants (MemoryMap.vhd; NPU_tb precedent: define
	-- locally rather than pulling in work.MemoryMap here)
	...
	-- FROZEN MCU/chip generics (npu_conv_design.md binding inputs; D9/G7:
	...
	-- gen_conv_vectors.py's golden model).
	...
	-- Bounded poll ceiling: largest predicted case (G-MULTI) is ~10.3k
	-- NpuClk cycles (D6); this is comfortably above every case with margin,
...
begin
	...
	SIM_PROCESS: process
		...
		-- has_x: definedness check (D9 G-DUP4). Written by hand (no Is_X
		-- dependency) so it compiles cleanly under -V200X.
		...
		-- calc_L: the same "exact valid conv, no padding" length formula
		-- gen_conv_vectors.py uses -- L is NOT stored in the cfg file (D9).
		...
		-- (exactly NPU_tb's own idiom -- waiting on MabSramCLK before CEN is
		-- ever asserted hangs forever, the gate never opens).
		...
		-- think_start: pokes IVSAR/WVSAR/OVSAR/NPUCFG1/NPUCFG2/NPUCR (MODE=
		-- CONV, THINK=1) for one case. l_calc is the derived input length.
		...
		-- think_poll: bounded poll of NPUCR bit16, also samples the DUT's
		-- own NpuSramGWEN_out each cycle (G-DEGEN L=0 "zero writes" proof).
		...
		-- run_conv_case: loads npu_conv_<case_name>_{cfg,in,w,exp}.txt (D9
		...
		begin
			...
			-- load inputs + weights in ONE SRAM burst (channel-major inputs
			-- from IVSAR, then filter-major weights from WVSAR)
			sram_burst_start;
	...
	begin
		...
		----------------------------------------------------------------
		-- G-BASE / G-DUP4 (X-collapse) / G-NEG: the FIRST thing this bench
		-- does after reset is run the base conv case, with no other CFG
		-- pokes ahead of it (D9 G-DUP4 -- proves the added conv registers
		-- cannot produce X on the SRAM interface after a cold reset) and
		-- every output checked for definedness. When NEGCTRL=1 this same
