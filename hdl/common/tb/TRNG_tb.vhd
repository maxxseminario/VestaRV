...
-- TRNG_tb.vhd
-------------------------------------------------------------------------------
-- Standalone, self-checking testbench for the TRNG peripheral (hdl/common/periph/TRNG.vhd), written AGAINST THE FROZEN ENTITY + REGISTER MAP (~/vesta_docs/digperiphs/trng_design.md, D1-D16 plus the ADJUDICATION RULINGS) while the RTL was written in parallel.
-- The DUT is declared as a COMPONENT (not an entity instantiation) so this bench compiles standalone with xmvhdl regardless of TRNG.vhd's analysis order; VHDL default binding resolves it to the entity of the same name once TRNG.vhd is analyzed into work.
-- Mirrors OneWire_tb.vhd / I2CTarget_tb.vhd structurally.
--
-- Uses tb/periph_tb_pkg.vhd (shared scoreboard + register-bus BFM) and tb/trng_bfm_pkg.vhd (slot/CR/SR/HT constants, the trng_mk_cr packer, the bounded SR/DRDY polls, and the checker-independent SEED/TAPS constants + single-step LFSR function).
-- The entropy source is tb/../periph/TrngRoEnsemble_sim.vhd (the sim architecture, D6), instantiated HERE as a companion model, exactly like every other protocol/entropy companion in this library (i2c_host_model, onewire_target_model, qspi_flash_model, nfc_reader_model, i3c_target_model).
-- It is wired through TRNG's genuine `ro_enable`/`ro_sel`/`ro_sclk`/`ro_raw` ports, never nested inside the DUT.
--
-- *** BINDING-PLACEMENT DISCREPANCY (flagged loudly; see TRNG.vhd's header for the full derivation) ***.
-- The task kickoff asserted "u_ro is INSIDE TRNG.vhd", implying this bench would need a nested VHDL `configuration` declaration to reach it.
-- Working through the FULL frozen material exposes a hard conflict with that reading, on three counts.
-- First, an entity `in` port cannot be driven by a component instantiated inside that same entity.
-- Second, the design doc's own G5 plan needs a hierarchical `force dut.u_ro.ro_raw`, which only parses when `dut` and `u_ro` are SIBLINGS under one common parent, exactly this TB, since -V200X has no VHDL-2008 external-name mechanism to reach two levels down.
-- Third, ruling 5's own literal text, "for u_ro : TrngRoEnsemble use entity work.TrngRoEnsemble(sim); in the BENCH ARCHITECTURE'S DECLARATIVE PART", is a plain VHDL *configuration specification*, which is only legal syntax when `u_ro` is a DIRECT component instantiation of THIS architecture, not nested a level deeper.
-- So `u_ro` is instantiated directly here, and the SAME plain configuration-specification form ruling 5 asks for is used below, verbatim, with no nested `configuration` unit needed.
-- Both this explicit form AND plain default binding (the behavioral flow only ever analyzes the `sim` architecture, so VHDL's sole-architecture rule would resolve it unaided) were proven this session; see the run log / report.
--
-- G5's "hierarchical force" (design doc bench plan) is realized here as a TB-owned override mux on the wire from `u_ro` into `dut` (`g5_force_en`/`g5_force_val`), rather than a literal `force`/`release` simulator command or a VHDL-2008 external name.
-- It is functionally identical (a stuck entropy bit reaching the DUT's `ro_raw` input) and fully -V200X-portable.
--
-- CHECKER INDEPENDENCE: the reference model (`ref_*` signals/processes below) is a clean-room re-implementation of TRNG.vhd's B2 (2-FF sync) + B3 (decimator + 32-bit assembler + D9 consume/pending), built ONLY from trng_bfm_pkg's SEED/TAPS constants and this TB's OWN shadow copies of what it commanded (EN/ROSEL/DECIM via `set_cr`, and DR-read requests via `read_dr`).
-- It NEVER reads a DUT-internal signal (dr_word, word_valid, asm_reg, trng_cr, ...).
-- Health-test/ALMF behavior is NOT modeled in the reference, a documented simplification: G0-G4/G7 never trigger an alarm by construction, since a Galois LFSR's bit-0 stream essentially never holds 32 identical decimated samples in a row, and G5, which deliberately DOES trigger one, checks DUT behavior directly rather than a value match.
--
-- ONE clock family (design doc D1/D2): `clk` (bound to MCLK at integration) hosts the whole harvest engine, and `ClkMem` is the gated bus clock, driven `clk when pbus.en_mem='0' else '0'` exactly like every other bench in this library.
-- No second domain (TRNG0 is mclk, D2).
--
-- EVFAB producer tap (event_fabric_spec.md 2026-07-24, EV13): TRNG.vhd grew one new output port, `evt_drdy`, which IS `drdy_level` (TRNG.vhd:281), the D9 blind-window-corrected DRDY LEVEL, pre-IE (DRDYIE never touches it).
-- Unlike the PULSE taps in PWM_tb.vhd/RTC_tb.vhd/UART_tb.vhd, which need a background start/width counter to prove one-clk-wide edges, this is an L-mode tap: the fabric does its own 2-FF plus rising-edge detect on a held level.
-- So GROUP G-EV below checks it the same way the rest of this bench already checks SR.DRDY, with point observations at the bench's own bus_read(SR)/read_dr settle points and no new monitor infrastructure.
-------------------------------------------------------------------------------
...
architecture sim of TRNG_tb is
    ...
    -- FROZEN DUT entity (design doc D3), declared as a component so the bench compiles standalone regardless of when TRNG.vhd is analyzed.
    component TRNG is
    ...
    -- Companion entropy-source model (D6), instantiated here like every other protocol/entropy companion in this library.
    ...
    -- ruling 5's explicit binding, verbatim: a plain configuration specification, legal here because u_ro is a direct instantiation of THIS architecture and not nested inside the DUT (see the header note).
    ...
    -- These are shadow copies of what the TB commanded, driven by set_cr/read_dr below.
    ---------------------------------------------------------------------------
...
begin
    ...
    -- G5 stuck-source override (header note): the value actually reaching dut.ro_raw is either u_ro's genuine output or a TB-forced constant.
    ...
    -- Clean-room mirror of TRNG.vhd's B2 (2-FF sync) + B3 (decimator + 32-bit assembler + D9 consume/pending), driven ONLY by this TB's OWN shadow signals (ref_en/ref_sel/ref_decim, updated by `set_cr` below) and its own record of qualifying DR reads (`ref_dr_read_req`, pulsed by `read_dr` below).
    ...
    -- B3 mirror: the sample-rate decimator, counting down the reload value and ticking at zero.
    ...
    -- D9 mirror: a qualifying read raises pending and flips the consume toggle, driven by ref_dr_read_req, which the `read_dr` procedure below pulses in lockstep with the real DR read.
    ...
    -- D9 mirror: 2-FF sync of the consume toggle plus the edge detect that becomes ref_consume_pulse.
    ...
    stim_proc : process
        ...
        -- Write TRNG0CR AND advance the reference model's shadow copies in lockstep, with the same cycle alignment as the DUT's own trng_cr commit (see the file header's checker-independence note).
        ...
        -- W1C the ALMF bit, then wait out the clr_almf_tgl 2-FF sync + edge detect (D8) before the caller relies on the flag being clear.
        ...
        -- D9 qualifying DR read, mirrored into the reference model the SAME cycle: ref_dr_read_req is asserted before the same rising edge the real bus transaction captures on (see the file header).
        ...
        -- Full chip-reset pulse, reusable between independent health-test sub-phases (G5/G6).
        -- The RCT run-length counter / have_prev state has no explicit re-arm event of its own in the design doc: only resetn clears it (D13).
        -- Carrying it across an EN disable/enable or an ALMF W1C would let a low RCTC from an earlier sub-test leak into a later one, since a stale high run_len can re-trip instantly against a new low cutoff.
        -- A clean resetn pulse between independent sub-tests keeps each one deterministic.
    ...
    begin
        ...
        ------------------------------------------------------------------
        -- GROUP G4: ROSEL plumbing (D6), a wiring/forwarding check.
        -- A NEW sel only takes effect the next time EN drops then rises again, because the sim stub perturbs its seed at that transition (D6 header).
        ------------------------------------------------------------------
        ...
        -- A clean resetn pulse before each independent sub-phase (see do_reset's comment): the RCT run-length counter has no re-arm event of its own besides resetn (D13), so a stale high run_len from an earlier sub-test could instantly re-trip a later, lower cutoff if left to carry over.
        do_reset;
        ...
        bus_read(clk, pbus, rdata_out, TRNG_SLOT_HT, rdw);
        -- The check is "at least", not "equal": the hand-off from decimator to health test is a registered 1-cycle pipeline, so a sample already in flight when alm_halt takes effect is still counted.
        -- RUNLEN therefore legitimately settles at cutoff or cutoff+1 (observed 9 for RCTC=8), never below cutoff.
        ...
        -- The run-length counter is still sitting at or above the old RCTC=8, so clearing ALMF while still forced-stuck would let harvesting resume for one cycle and immediately re-trip, the stale run_len already being at or above the still-active low cutoff.
        -- That is a testbench sequencing hazard, not a DUT defect.
        g5_force_en <= '0';
        ...
        -- RCTC override: confirm the DEFAULT cutoff (RCTC=0, i.e. 32) is HIGHER than the RCTC=8 override above, so a short stuck window of 10 samples must NOT trip it while continuing DOES.
        -- Fresh resetn first, for clean run_len/have_prev, same rationale as above.
        do_reset;
        ...
        -- leave it UNCONSUMED (DRDY stays pending), then force a fast alarm on top of it: the alarm only gates FUTURE samples (D8) and does not touch an already-completed, unconsumed dr_word.
        write_ht(x"08");
        g5_force_en  <= '1';
        g5_force_val <= '0';
        trng_wait_flag(clk, pbus, rdata_out, TRNG_SR_ALMF, '1', ok);
        ...
        -- Release the source and restore the default cutoff BEFORE the W1C (same sequencing rationale as G5), so the clear is genuine and lasting instead of instantly re-tripping against the stale run_len.
        g5_force_en <= '0';
        write_ht(x"00");
        w1c_almf;
        ...
        ------------------------------------------------------------------
        -- GROUP G7: NRO shape (D15).
        -- This WHOLE suite (G0-G6 above) has already run against the DUT + companion model elaborated at NRO_G, i.e. a generic override at elaboration such as NRO_G=4 for the {4,8} shape proof.
        -- The register map is NRO-invariant per the design doc, so an all-green run at NRO_G=4 IS the G7 proof.
        ------------------------------------------------------------------
        ...
        ------------------------------------------------------------------
        -- GROUP G-EV: EVFAB producer tap (evt_drdy, event_fabric_spec.md 2026-07-24 EV13).
        -- evt_drdy IS drdy_level (TRNG.vhd:281), the D9 blind-window-corrected DRDY LEVEL, pre-IE: DRDYIE has zero effect on it, unlike irq_trng which DRDYIE masks.
        -- It is checked at the SAME point observations the rest of this bench already uses for SR.DRDY (bus_read of SR, and the read_dr settle idiom), because a held LEVEL needs no start/width pulse monitor (contrast PWM_tb.vhd/RTC_tb.vhd/UART_tb.vhd's P/T-mode taps).
        ------------------------------------------------------------------
        report "=== GROUP G-EV: EVFAB producer tap (evt_drdy) ===" severity note;
        -- Fresh resetn first, same rationale as G5/G6's do_reset: a clean, deterministic starting state for an independent sub-phase.
        do_reset;
        write_ht(x"00");
        ...
        -- Exactly ONE deliberately-wrong expected value: expect a wrong entropy word after a clean, known harvest (the doc's own suggested example).
        ------------------------------------------------------------------
        report "=== GROUP G-NEG: NEGATIVE CONTROL ===" severity note;
        -- Fresh resetn first (same isolation rationale as G5/G6/G-EV's do_reset): a clean, known harvest, independent of whatever mid-cycle count the newly-added G-EV group left the assembler at, since asm_cnt/asm_reg only clear on resetn (D9/D10) and do NOT reset on an EN 0-to-1 toggle alone.
        -- ROSEL=0001 here, not the 0000 used everywhere else: the deterministic sel=0000 harvest from a clean reset lands bit31=1, which trips periph_tb_pkg.vhd's img() helper, because integer'image(to_integer(unsigned(...))) overflows VHDL's signed 32-bit integer range for values >= 2**31.
        -- That is a latent fragility in that shared, frozen Myshkin package, unrelated to TRNG.vhd; it was never hit before because an earlier, un-isolated G-NEG happened to ride in on stale asm_reg state that produced a bit31=0 word by luck.
        -- sel=0001's clean-reset word has bit31=0, which sidesteps the bug without touching periph_tb_pkg.vhd or any RTL.
        do_reset;
        write_ht(x"00");
