...
-- ntag5_model.vhd
-------------------------------------------------------------------------------
-- Behavioral simulation model of the NXP NTAG 5 dual-interface NFC tag, seen
-- FROM ITS I2C SLAVE SIDE ONLY. Castalia is the I2C MASTER (I2C0 on pins
-- 37/38, or I2C1 on pins 39/40); this model is the addressed slave, so a
-- board-level bench can bring up firmware against a plausible,
-- datasheet-grounded NTAG 5 without an RF stack.
--
...
-- Section / table numbers below are [L] unless marked [B]. Where the two
-- datasheets were compared byte-for-byte on a table, both citations are given.
--
-- ===========================================================================
-- VARIANT: WHAT ACTUALLY DIFFERS ON THE I2C SIDE -- NOTHING
-- ===========================================================================
-- The two datasheets were diffed on every I2C-visible item this model
-- implements and they AGREE EXACTLY:
...
-- The real differences are RF-side: NTA5332 uses ACTIVE load modulation with
-- its own VCC,TX / GNDTX supply pins ([B] 1, [B] Tab. 4), NTP53x2 uses passive
-- load modulation. This model does not implement the RF protocol at all, so
-- VARIANT changes NO behavior here. It is accepted, validated, and reported on
-- obs_variant_ok so a bench can prove the string was recognized -- rather than
-- inventing an I2C-side difference that does not exist.
--
-- ===========================================================================
-- WHAT THIS MODEL DOES IMPLEMENT
-- ===========================================================================
-- 1. I2C SLAVE bus protocol on an open-drain SDA/SCL pair ([L] 8.3.1.1):
--    START / repeated-START / STOP, 7-bit address + R/W bit, per-byte ACK/NACK,
--    MSB-first, sample on the SCL RISING edge ([L] 8.3.1.1.4), drive set up on
--    the SCL FALLING edge. Standard (100 kHz) and fast (400 kHz) mode are
--    supported in the sense that the model is fully edge-driven and carries NO
--    internal timing assumption at all -- it responds at whatever rate the
--    master clocks SCL. It never stretches SCL (see NOT IMPLEMENTED).
--
-- 2. THE ADDRESSING SCHEME -- the thing firmware trips on ([L] 8.3.1.4, Fig. 10
--    and Fig. 11). NTAG 5 does NOT use a flat byte address. Every access opens
--    with a 16-BIT BLOCK address, MSB first:
--       START, SL_AD+W, BL_AD1(MSB), BL_AD0(LSB), ...
--    A block is 4 bytes. The block address space seen from I2C is:
...
--       A THIRD address byte, REGA (0..3, the byte within the register block),
--       and on a write a MASK byte before the data:
...
--    The model decides a/b purely from the captured 16-bit block address, and
--    a bench that gets it wrong sees exactly the confusion real firmware sees.
--
...
--    Delivery content of user blocks 00h..05h is loaded per [L] Tab. 7 (the
--    NFC Forum capability container + the www.nxp.com/nfc NDEF message);
--    the rest of user EEPROM is 00h. The datasheet says the rest is random at
--    delivery and that "SRAM values are not initialized during boot and may
--    have arbitrary data" ([L] 8.1.5) -- this model uses 00h for both so runs
--    are deterministic. FLAGGED as a deliberate deviation.
--
--    DEFINED VALUE FOR EVERYTHING NOT MODELLED (the brief's requirement):
--      * every byte inside the four modelled ranges reads back what was
--        written to it, so "unimplemented config register" still round-trips;
--        config bytes that this model does not INTERPRET simply have no side
--        effect. RFU-only config blocks read 00h ([L] 8.1.3) because the whole
--        config array initialises to 00h.
--      * a READ of any block OUTSIDE the four ranges (0200h..0FFFh,
--        10B0h..1FFFh, >=2040h), of the counter block 01FFh, or of SRAM while
--        SRAM_ENABLE=0, returns FFh. That matches the datasheet's own rule for
--        a restricted region ([L] 8.3.1.5, "returned data will be FFh if the
--        access is to restricted region"); extending it to unmapped blocks is
--        an ASSUMPTION.
--      * a WRITE to an unmapped block is NACKed on its first data byte
--        (ASSUMPTION -- the datasheet enumerates NACK causes but not this one).
--
...
--    Decoded onto output ports eh_iout_ua / eh_vout_mv / eh_enabled /
--    eh_vout_on so a board bench can drive a supply model. The current codes
--    are datasheet MINIMA ("> 0.4 mA"); eh_iout_ua carries that minimum.
--    The RFU voltage code 11b drives eh_vout_mv = 0 (ASSUMPTION -- the part's
--    behavior for an RFU code is undefined) and eh_vout_rfu_v = '1'.
--    eh_vout_on models the actual pin: enabled AND a field is present AND
--    (DISABLE_POWER_CHECK or the bench says the field is strong enough).
--    Session-side EH_CONFIG_REG (block 10A7h byte 0, [L] Tab. 96/97) is
--    modelled read-only from I2C exactly as the datasheet's access column says
--    (NFC R/W, I2C R): a host write to it is silently dropped. EH_LOAD_OK
--    (bit 7) reads back rf_field AND rf_power_ok while session EH_ENABLE is 0,
--    and clears once EH is enabled ([L] Tab. 97 "As soon as EH_ENABLE is set
--    to 1b, this bit gets cleared automatically"). ASSUMPTION: the real part
--    requires an EH_TRIGGER write first, and EH_TRIGGER is writable only from
--    NFC, so this model reports EH_LOAD_OK without a trigger.
--
-- 5. EVENT DETECTION PIN ([L] 8.1.3.19 + Tab. 56, [L] 8.3.2). Open-drain,
--    ACTIVE LOW; the datasheet's "ED pin state ON" means the pin is pulled
--    LOW. Source = ED_CONFIG_REG(3:0) at session block 10A8h byte 0, POR-copied
--    from ED_CONFIG (config block 103Dh byte 2). Implemented sources:
...
--    Sources 2h (PWM), 5h (arbiter lock), 6h (NDEF TLV length), 7h (standby),
--    8h/9h/Ah (write/read/any-command indication), Bh/Ch (SYNCH_BLOCK access)
--    are NOT modelled and leave the pin released -- see NOT IMPLEMENTED.
--    ED_INTR_CLEAR_REG (block 10ABh byte 0, [L] Tab. 57/58): writing 01h
--    releases the pin and the bit self-clears.
--
-- 6. SRAM PASS-THROUGH ([L] 8.4.5, [L] 8.1.4.1 Tab. 87, [L] Tab. 91).
--    ARBITER_MODE = CONFIG_1_REG(3:2) at session block 10A1h byte 1
--    (10b = SRAM pass-through); PT_TRANSFER_DIR = CONFIG_1_REG(0)
--    (0 = I2C-to-NFC, 1 = NFC-to-I2C), both I2C-writable per [L] Tab. 91.
--    STATUS0_REG (block 10A0h byte 0, [L] Tab. 87) exposes
--    SRAM_DATA_READY(5) and a mirror of PT_TRANSFER_DIR(2), plus
--    EEPROM_WR_BUSY(7), EEPROM_WR_ERROR(6), VCC_SUPPLY_OK(1), NFC_FIELD_OK(0).
--    The datasheet says the arbiter switches automatically "when accessing the
--    terminator block (last block of SRAM)" and refers the handshake detail to
--    application note AN12364, which is NOT on disk. ASSUMPTION, stated
--    plainly: this model treats block 203Fh as the terminator and defines
--       NFC-to-I2C (dir=1): reader writes buffer -> SRAM_DATA_READY = 1;
--                           I2C host reading block 203Fh clears it
--       I2C-to-NFC (dir=0): I2C host writing block 203Fh -> SRAM_DATA_READY=1;
--                           the reader reading the buffer clears it
--    SRAM is only accessible while SRAM_ENABLE (CONFIG_1_REG(1)) is 1 ([L]
--    8.1.5); with it clear, SRAM reads return FFh and SRAM writes are NACKed
--    on the first data byte ([L] 8.3.1.5, "SRAM: generated on the first byte
--    if the block is not writable").
--
-- 7. WRITE PROTECTION ([L] 8.1.3.31 Tab. 79, [L] 8.3.1.5). I2C_LOCK_BLOCK
--    occupies config blocks 108Ah..1091h, bytes 0 and 1 of each: 16 bytes,
--    128 bits, each bit locking FOUR user blocks (bit 0 of I2C_LOCK_BL00 locks
--    user blocks 0,1,2,3). The bits are ONE-TIME PROGRAMMABLE, so this model
--    ORs writes into those bytes and never clears them. A WRITE MEMORY to a
--    locked user block is ACKed for data bytes 0..2 and NACKed on THE FOURTH
--    data byte, exactly as [L] 8.3.1.5 specifies for EEPROM.
--
-- 8. ERROR HANDLING ([L] 8.3.1.5), the NACK causes that are modelled:
--    * BL_AD0 is NACKed for a MEMORY access while an EEPROM programming cycle
--      is ongoing, or while the I2C interface is disabled (DISABLE_I2C,
--      I2C_SLAVE_CONFIG_REG(0), [L] Tab. 100 -- settable only from NFC, so a
--      bench pokes it through the rf_disable_i2c input).
--    * BL_AD0 is NACKed for a REGISTER access only when the I2C interface is
--      disabled -- registers are otherwise "always accessible".
...
--    * write to a config block that is RFU-only: NACKed ([L] 8.1.3, "Writing to
--      blocks with only RFU bytes is not possible and results in ... NAK from
--      I2C perspective"). The RFU block list encoded here is the one visible in
--      [L] Tab. 11: 100Ah, 100Bh, 100Fh, 1018h-101Fh, 1040h-1044h,
--      1059h-1069h, 109Ah-109Fh. PARTIAL by construction -- flagged.
--    * a REGISTER write of E7h to RESET_GEN_REG (block 10AAh byte 0,
--      [L] Tab. 101): the DATA byte is NACKed and a system reset is performed
--      ([L] 8.3.1.5 "DATA NACK due to register write command to trigger system
--      reset"). The model's reset re-does the POR copy of config -> session
--      registers and clears the ED/pass-through latches.
--    Once this model NACKs a byte it stays silent for the rest of the frame
--    (until the next START/Sr). ASSUMPTION -- the datasheet does not say, but
--    every I2C master aborts on a NACK anyway.
--
-- 9. RF-SIDE BENCH ABSTRACTION -- rf_* input ports. THIS MODEL DOES NOT
--    IMPLEMENT ISO/IEC 15693, NFC Forum Type 5 Tag, the NFC command set, AES
--    or plain-password authentication, or ANY air-interface behavior. The rf_*
--    ports exist ONLY so a testbench can say "a reader showed up and did X"
--    in one signal edge. They are a BENCH ABSTRACTION, not silicon:
...
--                      ([L] 8.3.1.1.6) -- I2C-side accesses then NACK
--
-- ===========================================================================
-- NOT IMPLEMENTED (deliberately)
-- ===========================================================================
--   * The NFC/RF air interface in every respect (see item 9).
--   * SCL clock stretching. [L] 8.3.1.1 describes an "asynchronous interface"
--     and never specifies a stretch; scl_out/scl_oe exist only to keep the
--     house open-drain port shape and are hardwired released.
--   * I2C MASTER mode ([L] 8.3.1.2) -- Castalia is the master on this bus.
--   * GPIO mode, PWM channels, the watchdog timer's timeout behavior
--     (WDT_ENABLE/WDT_CONFIG bytes are stored but never expire),
--     the arbiter's NFC-vs-I2C locking (I2C_IF_LOCKED / NFC_IF_LOCKED are
--     storage/constant, not a real arbiter), SRAM mirror and SRAM PHDC modes,
--     SRAM_COPY on POR, the 16-bit counter, standby / hard power-down.
--   * AES and plain-password authentication, and the I2C protection pointer
--     (I2C_PP/I2C_PPC) READ protection. Only the I2C_LOCK_BLOCK WRITE lock of
--     item 7 is enforced.
--   * Real EEPROM programming time. The datasheet extract specifies write
--     ENDURANCE (1e6 cycles) but no programming time, so EEPROM_PROG_TIME is a
--     GENERIC defaulting to 500 us -- an ASSUMPTION, tune it per bench.
--
-- ===========================================================================
-- BUS CONVENTION (house style, matches tb/i3c_target_model.vhd)
-- ===========================================================================
-- sda_out/sda_oe and scl_out/scl_oe: '1' on the *_oe port means THIS MODEL is
-- actively driving that pin (always to '0' -- open drain). The testbench
-- resolves the shared net, giving the released level a weak 'H' pull. This
-- model NEVER drives an active high on either pin; every sample is normalized
-- through to_X01 so a released 'H' reads as '1'.
-- ed_n_oe follows the same convention on the ED pin: '1' = this model pulls
-- ED low = the datasheet's "ED pin state ON".
-------------------------------------------------------------------------------
...
entity ntag5_model is
    generic (
        -- "NTP5332" (NTAG 5 link) or "NTA5332" (NTAG 5 boost). See the header:
        -- nothing on the I2C side differs between them.
    ...
    port (
        ...
        obs_pt_dir          : out std_logic := '0'; -- 0 = I2C->NFC, 1 = NFC->I2C
...
architecture behavioral of ntag5_model is
    ...
    ---------------------------------------------------------------------
    -- Per-byte I2C WRITE masks for the session registers ([L] Tab. 87, 88,
    -- 90, 91, 92, 97, 99, 100 access columns). A '1' means the I2C side may
    -- change that bit; a '0' means the datasheet marks it R from I2C, and a
    -- host write to it is silently dropped.
    ...
    ---------------------------------------------------------------------
    -- Configuration-memory reset content. Only the bytes this model
    -- interprets are given non-zero defaults; the rest are 00h.
    ---------------------------------------------------------------------
    function cfg_init return byte_arr_t is
        variable c : byte_arr_t(0 to CFG_BLOCKS*4 - 1) := (others => x"00");
    begin
        -- CONFIG_0 ([L] Tab. 37): EH_MODE = 10b "optimized for low field
        -- strength" is the stated default -> bits 3:2 = 10 -> 08h.
        c(CFO_CONFIG*4 + 0) := x"08";
        -- CONFIG_1 ([L] Tab. 38): all defaults are 0 (I2C slave, normal
        -- arbiter mode, SRAM disabled, PT direction I2C->NFC).
        c(CFO_CONFIG*4 + 1) := x"00";
        -- CONFIG_2 ([L] Tab. 39): EXTENDED_COMMANDS_SUPPORTED,
        -- LOCK_BLOCK_COMMAND_SUPPORTED and both slew-rate bits default to 1.
        -- NOTE: the GPIOx_IN default is self-contradictory in the datasheet
        -- (both 00b and 11b are marked "(Default)"); GPIO is not modelled, so
        -- the upper nibble is left 0.
        c(CFO_CONFIG*4 + 2) := x"0F";
        -- WDT_ENABLE ([L] Tab. 51): default 1b = watchdog enabled.
        c(CFO_WDT*4 + 2) := x"01";
        -- EH_CONFIG / ED_CONFIG ([L] Tab. 53 / Tab. 56): all defaults are 0
        -- (>0.4 mA, power check on, 1.8 V, EH disabled; ED disabled).
        c(CFO_EHED*4 + 0) := x"00";
        c(CFO_EHED*4 + 2) := x"00";
    ...
    ---------------------------------------------------------------------
    -- POR copy config -> session registers ([L] 8.1.4: "After POR, the
    -- content of the configuration settings is loaded into the session
    -- register"). The block pairing is from [L] Tab. 11 vs Tab. 85.
    ---------------------------------------------------------------------
    function reg_init(c : byte_arr_t) return byte_arr_t is
        variable r : byte_arr_t(0 to REG_BLOCKS*4 - 1) := (others => x"00");
    begin
        for i in 0 to 2 loop
            r(RGO_CONFIG*4 + i) := c(CFO_CONFIG*4  + i);   -- A1h <- 1037h
            r(RGO_SYNC*4   + i) := c(CFO_SYNC*4    + i);   -- A2h <- 1038h
            r(RGO_PWMGP*4  + i) := c(CFO_PWMGPIO*4 + i);   -- A3h <- 1039h
            r(RGO_WDT*4    + i) := c(CFO_WDT*4     + i);   -- A6h <- 103Ch
        end loop;
        for i in 0 to 3 loop
            r(RGO_PWM0*4 + i) := c(CFO_PWM0*4 + i);        -- A4h <- 103Ah
            r(RGO_PWM1*4 + i) := c(CFO_PWM1*4 + i);        -- A5h <- 103Bh
        end loop;
        r(RGO_ED*4     + 0) := c(CFO_EHED*4   + 2);        -- A8h <- ED_CONFIG
        r(RGO_I2CSLV*4 + 0) := c(CFO_I2CSLV*4 + 0);        -- A9h <- slave addr
        r(RGO_I2CSLV*4 + 1) := c(CFO_I2CSLV*4 + 1);        -- A9h <- slave config
        return r;
    end function;

    -- RFU-only configuration blocks ([L] Tab. 11) -- writes are NACKed.
    ...
    -- EEPROM programming-cycle flag: a signal so the process can schedule its
    -- own clear with an `after`.
...
begin
    ...
    ---------------------------------------------------------------------
    -- One process owns the bus FSM, all four memories, the RF stimulus
    -- reactions and every derived output. Memories live in process
    -- variables; the derived outputs (ED pin, EH decode, observation) are
    -- recomputed unconditionally at the BOTTOM of every activation.
    ---------------------------------------------------------------------
    slave : process (scl, sda_in, rf_field, rf_power_ok,
        ...
        -- ---- addressing state (PERSISTS across STOP -- see header 2) ----
    ...
    begin
        -------------------------------------------------------------------
        -- RF-SIDE BENCH STIMULUS (not the air interface -- see header 9)
        -------------------------------------------------------------------
        if rf_write_sram'event and to_X01(rf_write_sram) = '1' then
            ...
            if regs(RGO_CONFIG*4 + 1)(0) = '1' then   -- PT dir = NFC -> I2C
                sram_rdy := '1';
        ...
        if rf_read_sram'event and to_X01(rf_read_sram) = '1' then
            ...
            if regs(RGO_CONFIG*4 + 1)(0) = '0' then   -- PT dir = I2C -> NFC
                sram_rdy := '0';
                ed_i2c   := '1';   -- "last byte read via NFC; host can access
                                   --  SRAM again" ([L] Tab. 56, source 3h)
            end if;
        end if;
        ...
        if sda_in'event and to_X01(scl) = '1' then
            ...
            if to_X01(sda_in) = '0' then
                ...
                -- I2C_S_REPEATED_START ([L] Tab. 100 bit 2): when set, the
                -- slave "resets internal state machine on repeated start".
                ...
                -- STOP. Commit any staged EEPROM/config block write; the
                -- datasheet ties the internal write cycle to the STOP
                -- ([L] 8.3.1.1.2).
                if active then
                    if stg_pend then
                        if stg_blk <= BLK_USER_LAST_RW then
                            ...
                            for i in 0 to 3 loop
                                ...
                                if stg_blk >= 16#108A# and stg_blk <= 16#1091#
                                   and i <= 1 then
                                    -- I2C_LOCK_BLOCK bytes are ONE-TIME
                                    -- PROGRAMMABLE ([L] 8.1.3.31): set only.
                                    cfg(idx) := cfg(idx) or stg(i);
                                else
                                    cfg(idx) := stg(i);
                                end if;
                            end loop;
                            -- ED software-interrupt source armed by writing
                            -- Dh into ED_CONFIG ([L] Tab. 56, source Dh).
            ...
            ------------------------------------------------------------
            -- SCL FALLING edge: process + set up this model's next drive
            -- (edge has already been incremented, so g/bp name the
            -- UPCOMING sample slot -- the tb/i3c_target_model idiom).
            ------------------------------------------------------------
            g  := edge / 9;
            bp := edge mod 9;
            ...
            if read_done or frame_dead then
                ...
                if bp = 8 then
                    ...
                    if addr_ok then
                        ...
                        -- "If the IC does not match the device select code, it
                        -- deselects itself from the bus" ([L] 8.3.1.1.5)
                        obs_addr_acked <= '0';
                        nack_v := nack_v + 1;
                ...
                if is_read then
                    ...
                    if bp = 0 then
                        ...
                        if not ptr_valid then
                            ...
                            -- REGISTER read: the datasheet's frame carries
                            -- exactly one REGDAT byte. ASSUMPTION: further
                            -- reads without a new pointer repeat that byte.
                            ...
                            -- MEMORY reads auto-increment and "respond until
                            -- host's NACK" ([L] 8.3.1.4)
                            if ptr_byt = 3 then
                                ptr_byt := 0;
                                ptr_blk := ptr_blk + 1;
                            else
                            ...
                            -- pass-through consume: the I2C host reading the
                            -- terminator block clears SRAM_DATA_READY
                    ...
                    if bp /= 8 then
                        ...
                        if n = 0 then
                            ...
                            -- BL_AD0 (LSB) -- the whole block address is known
                            ...
                            if n = 2 then
                                ...
                                if ptr_blk = BLK_REG_BASE + RGO_RESET
                                   and reg_rega = 0 and shift = x"E7" then
                                    -- system reset ([L] Tab. 101): DATA NACK
                                    -- ([L] 8.3.1.5) then redo the POR copy.
                                    ack_v    := false;
                                    regs     := reg_init(cfg);
                                    ed_i2c   := '0';
                                    ed_nfc   := '0';
                                    ...
                                    if ptr_blk = BLK_REG_BASE + RGO_EDCLR
                                       and reg_rega = 0 and newb(0) = '1' then
                                        -- "write 1b to release ED pin"; the bit
                                        -- self-clears ([L] Tab. 58)
                                        ed_i2c := '0';
                                        ed_nfc := '0';
                                        ed_swi := '0';
                                        regs(idx)(0) := '0';
                            ...
                            if ptr_blk >= BLK_SRAM_BASE and ptr_blk <= BLK_SRAM_TERM then
                                ...
                                -- "generated on the first byte if the block is
                                -- not writable" ([L] 8.3.1.5)
                                if regs(RGO_CONFIG*4 + 1)(1) = '0' then
                                    ...
                                    if ptr_blk = BLK_SRAM_TERM and ptr_byt = 3
                                       and regs(RGO_CONFIG*4 + 1)(0) = '0' then
                                        -- I2C -> NFC: writing the terminator
                                        -- block hands the buffer to the reader
                                        sram_rdy := '1';
                                        ed_i2c   := '0';
                                    end if;
                                    if ptr_byt = 3 then
                                ...
                                -- EEPROM / configuration: exactly ONE 4-byte
                                -- block per frame ("N shall be 3", [L] 8.3.1.4)
                                if d > 3 then
                                    ack_v := false;
                                else
                                    stg(d) := shift;
                                    if d = 3 then
                                        -- writability decided on the FOURTH
                                        -- data byte ([L] 8.3.1.5)
                                        ack_v := true;
                                ...
                                -- unmapped block (ASSUMPTION: NACK, header 3)
                                ack_v := false;
                            end if;
                        end if;
        ...
        -------------------------------------------------------------------
        -- Energy harvesting ([L] Tab. 53). The V/I selections live ONLY in the
        -- config byte; the session register carries just EH_ENABLE/LOAD_OK.
