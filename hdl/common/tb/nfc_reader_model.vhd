-- VestaRV: NFC reader model
-- behavioral ISO 14443A reader (PCD) model closing the RF link for the NFC card-emulation testbench, sharing no encode or decode logic with the DUT beyond the spec-literal CRC_A and parity helpers in nfc_bfm_pkg, and taking a per-transaction cfg_* shape held stable across one frame.
