...
architecture testbench of NPU_gemm_tb is
	...
	-- FROZEN MCU/chip generics: the NPU entity's own defaults are the bench config Q0.15/Q3.15, and using those here would diverge from silicon and from the fixed-point encoding of gen_gemm_vectors.py's golden model.
	...
	-- Bounded poll ceiling: the largest case (maxfit, M=K=N=36) takes 1 + 1296*181 + 2 = 234579 NpuClk cycles, so this clears it with ~11% margin while staying far short of the 100 ms / 2.4M-cycle tb watchdog.
...
begin
	...
	SIM_PROCESS: process
		...
		-- think_start: pokes IVSAR/WVSAR/OVSAR/NPUCFG1(M-1)/NPUCR for one case.
		...
		-- think_poll: bounded poll of NPUCR bit 16 (THINK); GEMM has no zero-iteration path, so there is no "zero writes" side-channel to sample.
		...
		/* run_gemm_case: loads npu_gemm_<case_name>_{cfg,in,w,exp}.txt, runs the THINK, compares every output, and writes npu_gemm_<case_name>_act.txt with the actuals for post-mortem.
		     check_no_x : also assert each output is fully defined.  corrupt_mid : deliberately corrupt the MID-POSITION expected value, index (M*N)/2 with integer division, for the negative control.
