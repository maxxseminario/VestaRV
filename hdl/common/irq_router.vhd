-- VestaRV: interrupt router
-- THE peripheral interrupt controller: per-hart routing rows plus PLIC-style CLAIM/COMPLETE delivery, at page 3 of the shared peripheral window (0x7000) as a native arbiter slave on the free-running mclk, so meip can wake a hart whose gated clk_cpu is off.
-- Delivery is ONE registered wire per hart: meip(h) = OR over peripheral sources i of (level(i) AND en[h](i) AND NOT in_service(i)), every row live including row 0. The CLINT source IDs are excluded because msip and mtip are per-hart hardwired wires; their row bits are writable but inert.
-- CLAIM (read 0x7800) atomically returns the lowest pending source ID enabled for the READING hart, attributed by the arbiter's granted master, and sets in_service(id) so the source leaves EVERY hart's meip until completed: exactly-once delivery. With nothing pending it returns CLAIM_NONE, which the dispatcher treats as spurious. Priority is fixed lowest-ID, so a chatty low-ID source can starve higher IDs on the same hart.
-- COMPLETE (write the ID to 0x7800) clears in_service(id). It is owner-UNQUALIFIED, so any hart can recover a hung hart's claim; IDs at or above NUM_SRCS are ignored, so a stashed CLAIM_NONE is a no-op; a source whose level is still high re-pends.
-- Enable rows are NUM_EN_WORDS = ceil(NUM_SRCS/32) words per hart at word 4h+k, contiguously packed both ways. Everything unmapped reads 0 and ignores writes, as do row indices at or above NHARTS.
