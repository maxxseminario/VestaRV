-- VestaRV: TRNG ring-oscillator ensemble, behavioral model
-- A deterministic 32-bit Galois LFSR standing in for the ring oscillators, whose combinational loops delta-loop a behavioral simulator into zero-time oscillation.
-- Never co-listed with TrngRoEnsemble.vhd, which carries the real rtl architecture of the same entity; genus and gate cell lists must compile that file and never see this one.
