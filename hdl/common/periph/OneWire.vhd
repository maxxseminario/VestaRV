-- VestaRV: 1-Wire master
-- Dallas/Maxim 1-Wire master, one open-drain DQ pin, one combined IRQ, register base 0x6700.
...
-- The protocol logic rides the free-running clk and the register file the gated ClkMem; every hand-off is a toggle or a quasi-static level, and EnMemPeriph is an active-low qualifying level, never a clock or an edge.
...
architecture behavioral of OneWire is
    ...
    /* Slot timing in 0.5 us ticks; OW0DIV = 11 at 24 MHz gives one tick per 12 clk cycles.
       Reset slot: tRSTL low, release, presence sampled tPRES later, then tRSTH high. Write and read slots: drive low, release to tSLOT, then tREC.
       Overdrive tMSR is 3 ticks, not 4, because 4 would sample exactly at the 2 us tRDV edge.
