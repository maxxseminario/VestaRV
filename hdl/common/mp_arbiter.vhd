-- VestaRV: shared-bus arbiter
-- Round-robin serializing arbiter, N masters (harts) onto ONE single-port shared slave. Exactly one master is granted at a time and the grant is held for its whole multi-cycle transaction, so a data cycle can never latch another master's access.
