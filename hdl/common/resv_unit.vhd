-- VestaRV: LR/SC reservation unit
-- Global reservation table for the shared-RAM window, between mp_arbiter's slave port and the shared RAM, on the free-running mclk. An SC to shared memory must be adjudicated here, in the arbiter's serialization order: a per-hart local reservation flop lets two harts pass their own check and both report success.
...
-- lr_sc per master: "01" = LR read, "10" = SC write attempt, "00" = plain access, stable for the whole transaction because the issuing core is frozen, and gated by the master's req.
