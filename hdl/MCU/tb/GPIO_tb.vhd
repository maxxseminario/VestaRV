...
-- (hdl/MCU/periph/GPIO.vhd), configured like the SoC's GPIO1 port:
...
architecture sim of GPIO_tb is
    ...
    -- Alternate-function drivers
    signal alt_func_out_in : pinvec := (others => '0');
    signal alt_func_dir_in : pinvec := (others => '0');
    signal alt_func_ren_in : pinvec := (others => '0');
    ...
    -- value, the SEL mux, and the pad logic polarity.
    function pad_route(reg, alt, sel : pinvec; pos_logic : boolean) return pinvec is
        variable r : pinvec;
    begin
...
begin
    ...
    stim_proc : process
        ...
        -- current PxOUT/PxDIR/PxREN, SEL, and alt-function inputs.
        procedure check_pads(tag : in string;
                             pxout, pxdir, pxren, sel : in pinvec) is
        begin
            check_slv(tag & ": prt_out_out",
                      prt_out_out, pad_route(pxout, alt_func_out_in, sel, OUT_POS));
            check_slv(tag & ": prt_dir_out",
                      prt_dir_out, pad_route(pxdir, alt_func_dir_in, sel, DIR_POS));
            check_slv(tag & ": prt_ren_out",
                      prt_ren_out, pad_route(pxren, alt_func_ren_in, sel, REN_POS));
        end procedure;
    ...
    begin
        ...
        bus_write8(RegSlotPxDIR, x"00");
        bus_write8(RegSlotPxREN, x"00");
        alt_func_out_in <= x"A0";
        alt_func_dir_in <= x"50";
        alt_func_ren_in <= x"30";
        wait for 2 * CLK_PERIOD;
