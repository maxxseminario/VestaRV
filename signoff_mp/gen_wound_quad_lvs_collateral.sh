...
# chip_top_wound_quad signoff block. CHIP-ONLY clone of
# gen_wound_lvs_collateral.sh -- there is NO block leg here, because
# chip_top_wound_quad is a FLAT chip run: the MCU_WOUND hierarchy is placed and
...
#
# WHAT IT PRODUCES (exactly what signoff_mp/Makefile's chip_top_wound_quad
# NETLIST/LABELS point at):
#   pvs/chip_top_wound_quad.lvs.v        (chip only, from the cut's own DB)
#   pvs/chip_top_wound_quad.lvslabels    (same session, VDD_SW_H0..3, 4-orient)
#   pvs/chip_top_wound_quad_full.lvs.v   = patched chip + pvs/hart_tile.lvs.v
#
...
# as argv[1], and chip_top_wound_quad shares the very same padlist file
...
#
# usage: ./gen_wound_quad_lvs_collateral.sh [skip_netlist]
#   skip_netlist = reuse the existing pvs/chip_top_wound_quad.lvs.v + labels,
#                  redo the pad patch + concat only.
#
# Prereqs: source ~/vestarv/cdspaths.sh (innovus on PATH); the wound-quad cut's
# ../innovus/common/chip_top_wound_quad/dbs/chip_top_wound_quad.signoff.innovus.dat +
# out/chip_top_wound_quad.xsim.v present; pvs/hart_tile.lvs.v and
...
SKIP=${1:-}
INN=../innovus/common/chip_top_wound_quad   # per-block layout
PVS=pvs
TILE_NETLIST=$PVS/hart_tile.lvs.v
BASE=chip_top_wound_quad
...
echo "  make drc BLOCK=chip_top_wound_quad"
echo "  make ant BLOCK=chip_top_wound_quad"
echo "  make lvs BLOCK=chip_top_wound_quad   # LABELS default pvs/chip_top_wound_quad.lvslabels"
