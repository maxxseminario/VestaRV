# C6 endgame PASS 1e (2026-07-29) — FINAL pass-1 flavor.
...
#   - Count guards; trial streamOut out/MCU_castalia.c6e1e.gds2; NO saveDesign.
#
# Run: cd ~/vestarv/innovus/common/MCU_castalia && innovus -no_gui -batch \
#        -log log/mcu_castalia_c6e1e \
#        -files /home/mseminario2/vestarv/signoff_mp/tcl/mcu_castalia_c6_pass1e.tcl
...
__swire_boxdump VSS M2 $REPORT_DIR/MCU_castalia.c6e1e.vssm2_pre.txt
puts "C6E1: VSS/M2 box dump PRE written"
...
__swire_boxdump VSS M2 $REPORT_DIR/MCU_castalia.c6e1e.vssm2_post.txt
puts "C6E1: VSS/M2 box dump POST written"
...
dict for {k v} $__sw_pre {
	...
	if {$__shr > 5.0 && $__shr > 0.01 * $a0} {
		lassign [split $k |] n l shp
		if {$shp eq "shield"} {
			puts [format "C6E1 SHIELD-SHRINK (accepted) %s: area %.2f -> %.2f (count %d -> %d)" $k $a0 $a1 $c0 $c1]
		} else {
...
puts "C6E1: sWire classifier PASS (area-conserving; shield rip/regen + fragmentation logged only)"
...
# ---------------- trial streamOut -------------------------------------------
streamOut \
	$OUTPUT_DIR/MCU_castalia.c6e1e.gds2 \
	-libName WorkLib \
...
puts "C6E1: COMPLETE (trial GDS out/MCU_castalia.c6e1e.gds2, no saveDesign)"
exit
