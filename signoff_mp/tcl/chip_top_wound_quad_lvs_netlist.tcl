# Wound-QUAD chip (staged 2026-07-27, NOTHING RUN YET): chip_top_wound_quad
...
# (out/chip_top_wound_quad.xsim.v) -- the flow INPUT verilog lacks the chip-level
...
#   1. chip_top_wound -> chip_top_wound_quad; viewdefinition_chip_wound ->
#      _chip_wound_quad. LEF set is byte-identical to the wound/DP chips'
#      (verified against tcl/chip_top_wound_quad.innovus.tcl: same 8 entries
#      incl. the same tphn65gpgv2od3_sl_8lm IO_PAD_LEF path).
#   2. saveNetlist target is the CHIP-ONLY netlist pvs/chip_top_wound_quad.lvs.v.
...
#      Pad patch + concatenation live in gen_wound_quad_lvs_collateral.sh.
#   3. HARD sanity gate instead of the DP comment-only "should": distinct FE_OFN*
#      and CTS_* counts must equal out/chip_top_wound_quad.xsim.v; on mismatch the
#      netlist is DELETED and the script exits 1.
#   4. Same-session pvs/chip_top_wound_quad.lvslabels dump (G0). NO staged copy
...
#      pvs/lvs_chip_wound_quad_ctl -- keep names and cpoints in sync.
#      NB lvs.sh ALSO injects pvs/hart_tile_vddsw.lvslabels into the boxed
#      hart_tile struct for any cell matching chip_top* -- chip_top_wound_quad
#      does.
#
# Run: cd ~/vestarv/innovus/common/chip_top_wound_quad && innovus -no_gui -batch \
#        -log log/chip_top_wound_quad_lvs_regen -files ../../../signoff_mp/tcl/chip_top_wound_quad_lvs_netlist.tcl
# Normally invoked through ../../signoff_mp/gen_wound_quad_lvs_collateral.sh chip.
...
set XSIM_REF      "$OUTPUT_DIR/chip_top_wound_quad.xsim.v"
set NETLIST_OUT   "$PVS_DIR/chip_top_wound_quad.lvs.v"
set LABELS_OUT    "$PVS_DIR/chip_top_wound_quad.lvslabels"
...
set init_top_cell  chip_top_wound_quad
set init_pwr_net   VDD
set init_gnd_net   VSS
set init_mmmc_file "$SCRIPT_DIR/viewdefinition_chip_wound_quad.tcl"
...
defIn $DATABASE_DIR/chip_top_wound_quad.signoff.innovus.dat/chip_top_wound_quad.def.gz
clearGlobalNets
...
if {[file mtime $TILE_LABELS] < [file mtime $OUTPUT_DIR/hart_tile.gds2]} {
    puts "WQCHIPREGEN: FATAL: $TILE_LABELS is older than $OUTPUT_DIR/hart_tile.gds2 --"
...
puts "WQCHIPREGEN: DONE (netlist + same-cut labels).  Now run gen_wound_quad_lvs_collateral.sh"
