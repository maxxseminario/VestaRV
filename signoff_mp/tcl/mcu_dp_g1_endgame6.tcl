# G1 endgame PASS 5 (2026-07-23) — MCU_DP full endgame on the defIn vehicle.
...
# boundary hops, disjoint from the ripped cones). streamOut out/MCU_DP.g1e6.gds2.
...
#        -log log/mcu_dp_g1e6 -files ../../../signoff_mp/tcl/mcu_dp_g1_endgame6.tcl
...
if {[expr {$__ns0 - $__ns1}] > 40 || $__ns1 > $__ns0} { __fatal "sWire delta on rip out of bounds ($__ns0 -> $__ns1)" }
if {$__nw1 >= $__nw0 || [expr {$__nw0 - $__nw1}] > 3000} { __fatal "wire rip delta implausible ($__nw0 -> $__nw1)" }
...
# ---------------- surgical merges (POST-ecoRoute: patches must be INVISIBLE
# to the router — pass-4 proved a pre-route merge makes ecoRoute reroute the
# host net, orphaning the patches; PG4 order restored) ----------------------
...
if {$__fatals > 0} { __fatal "$__fatals merge fatals" }


# the 881 pin-access stub (ecoRoute must land irq_comb_48 on a cell pin inside
# the dm2 blockage; the landing is < min-area every pass — extend DOWN, away
# from the dummy band above)
# via-aware query: the stub is a VIA LANDING PAD — invisible to wire/sWire
# queries (the PG4 analog-band lesson)
set __n881 [__netat M2 {881.38 480.39 881.52 480.61}]
if {$__n881 eq ""} {
	foreach __o [dbQuery -area {881.38 480.39 881.52 480.61} -objType via] {
		set __bl [dbGet -e $__o.via.botLayer.name]
		set __tl [dbGet -e $__o.via.topLayer.name]
		if {$__bl ne "M2" && $__tl ne "M2"} { continue }
		set __n881 [lindex [dbGet -e $__o.net.name] 0]
		if {$__n881 ne ""} { break }
	}
}
if {$__n881 ne ""} {
	puts "G1E3: 881 pin-stub net = $__n881 (via-aware)"
	__mrg "M2.A.1@881pin" $__n881 M2 {881.40 479.99 881.50 480.41}
} else {
	__fatal "881 pin-stub net not found even via-aware"
}
if {$__fatals > 0} { __fatal "$__fatals post-route merge fatals" }
...
# finalize collateral to NON-canonical paths — promotion is a shell-side cp
# AFTER Calibre passes on the trial GDS
saveDesign $DATABASE_DIR/MCU_DP.g1.innovus -def -netlist -rc -tcon
write_sdf $OUTPUT_DIR/MCU_DP.g1.sdf
puts "G1E3: COMPLETE (trial GDS out/MCU_DP.g1e6.gds2 + DB MCU_DP.g1.innovus + SDF MCU_DP.g1.sdf)"
exit
