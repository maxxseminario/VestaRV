# C6 endgame PASS 2 (2026-07-29) — eco REPLAY (proven deterministic x3) +
...
# M2.S.1 cell-internal site. Trial streamOut c6e2, NO saveDesign.
# (pass-1e lineage:
...
#   - Count guards; trial streamOut out/MCU_castalia.c6e2.gds2; NO saveDesign.
#
# Run: cd ~/vestarv/innovus/common/MCU_castalia && innovus -no_gui -batch \
#        -log log/mcu_castalia_c6e2 \
#        -files /home/mseminario2/vestarv/signoff_mp/tcl/mcu_castalia_c6_pass2.tcl
...
proc __fatal {msg} { puts "C6E2 FATAL: $msg"; exit 1 }
foreach {__ni0 __ns0 __nw0} [__counts] {}
puts "C6E2 COUNTS baseline: insts=$__ni0 sWires=$__ns0 wires=$__nw0"
...
puts "C6E2 route blockages carried by DEF: $__rblk0 (expect seal-ring only)"
...
puts "C6E2 sWire snapshot PRE: [dict size $__sw_pre] (net,layer) pairs"
__swire_boxdump VSS M2 $REPORT_DIR/MCU_castalia.c6e2.vssm2_pre.txt
puts "C6E2: VSS/M2 box dump PRE written"
...
puts "C6E2: routing-era blockades re-declared (M7/M8 blankets + 4 windows)"
...
puts "C6E2: cut route blockages created (M3 x2, M5 x1)"

if {[catch {ecoRoute -fix_drc} r]} { puts "C6E2 WARN: ecoRoute -fix_drc: $r" }
puts "C6E2: ecoRoute -fix_drc done"
...
__swire_boxdump VSS M2 $REPORT_DIR/MCU_castalia.c6e2.vssm2_post.txt
puts "C6E2: VSS/M2 box dump POST written"

foreach {__ni1 __ns1 __nw1} [__counts] {}
puts "C6E2 COUNTS post-eco: insts=$__ni1 sWires=$__ns1 wires=$__nw1 (base $__ni0/$__ns0/$__nw0)"
...
puts "C6E2 sWire snapshot POST: [dict size $__sw_post] pairs (was [dict size $__sw_pre])"
...
puts [format "C6E2 PG (VDD+VSS) sWire area: pre=%.1f post=%.1f um^2 (drift %.4f%%)" \
...
dict for {k v} $__sw_pre {
	...
	if {$__shr > 5.0 && $__shr > 0.01 * $a0} {
		lassign [split $k |] n l shp
		if {$shp eq "shield"} {
			puts [format "C6E2 SHIELD-SHRINK (accepted) %s: area %.2f -> %.2f (count %d -> %d)" $k $a0 $a1 $c0 $c1]
		} else {
			puts [format "C6E2 SHRINK %s: area %.2f -> %.2f (count %d -> %d)" $k $a0 $a1 $c0 $c1]
			incr __nshr
		}
	}
}
dict for {k v} $__sw_post {
	...
	lassign [split $k |] n l shp
	lassign $v c1 a1
	puts [format "C6E2 NEWPAIR %s: count=%d area=%.2f" $k $c1 $a1]
...
puts "C6E2: sWire classifier PASS (area-conserving; shield rip/regen + fragmentation logged only)"


# ================== ADDITIVE STAGE (after eco replay) =======================
set __fatals 0
foreach {__nie __nse __nwe} [__counts] {}
puts "C6E2 COUNTS post-eco: insts=$__nie sWires=$__nse wires=$__nwe"
...
proc __minarea_pass {sitesfile tag} {
	...
	while {[gets $__fp __line] >= 0} {
		...
		if {$__net eq ""} { incr __nskip; puts "C6E2 minarea($tag): NO wire under $__lay ($__x0 $__y0 $__x1 $__y1) — skipped"; continue }
		set __done 0
		...
		if {!$__done} { incr __nfail; puts "C6E2 minarea($tag): BLOCKED both directions at $__lay ($__x0 $__y0 $__x1 $__y1) net=$__net" }
	}
	close $__fp
	puts "### C6E2 ### minarea $tag — $__npatch patched, $__nskip skipped, $__nfail blocked"
...
proc __mrg {tag net lay rect {clr 0.16}} {
	...
	foreach o [concat [dbQuery -area $chk -objType wire] [dbQuery -area $chk -objType sWire]] {
		...
		if {$dx < $thr && $dy < $thr} {
			puts "C6E2 FATAL: $tag foreign $lay net=[dbGet -e $o.net.name] at $ob blocks rect $rect"
			incr __fatals
			return
		}
	}
	add_shape -net $net -layer $lay -rect $rect -shape STRIPE -status ROUTED
	puts "C6E2: $tag merged rect $rect on $net/$lay"
}
...
proc __mrg_at {tag lay rect {clr 0.16}} {
	...
	if {$n eq ""} { global __fatals; puts "C6E2 FATAL: $tag no $lay net near $rect"; incr __fatals; return }
...
# M2.S.2-family same-net sliver fills (single-net windows, c6e1e dumps)
__mrg "M2S2@708a"  mcu0/CTS_30              M2 {708.0   1023.885 708.1  1024.905}
__mrg "M2S2@708b"  mcu0/CTS_30              M2 {708.0   1025.295 708.1  1025.705}
__mrg "M2S2@1297"  mcu0/nfc0/CTS_6          M2 {1297.5  1725.5   1297.6 1726.135}
__mrg "M2S2@1320"  mcu0/i3c0/rc_gclk_19685  M2 {1320.0  937.095  1320.1 938.115}
__mrg "M2S2@1645"  mcu0/nfc0/CTS_9          M2 {1645.0  1681.095 1645.1 1682.115}
__mrg "M2S21@1148" mcu0/timer1/rc_gclk_3530 M2 {1148.75 851.895  1148.9 852.3}
	...
	foreach sv [dbGet -e $__pgnets($pg).sVias] {
		...
		foreach w $__svwins {
			...
			if {$bx0 <= $wx1 && $bx1 >= $wx0 && $by0 <= $wy1 && $by1 >= $wy0} {
				puts "C6E2 SVIA $tag net=$pg via=[dbGet -e $sv.via.name] box=$b status=[dbGet -e $sv.status]"
			}
		}
	}
}
puts "C6E2: sVia dumps done"
# Insts at the M2.S.1 cell-internal site
foreach o [dbQuery -area {632.4 784.3 634.4 786.7} -objType inst] {
	puts "C6E2 M2S1INST name=[dbGet -e $o.name] cell=[dbGet -e $o.cell.name] box=[dbGet -e $o.box] orient=[dbGet -e $o.orient]"
}
puts "C6E2: M2.S.1 inst dump done"

# ---------------- guards + trial streamOut ----------------------------------
if {$__fatals > 0} { __fatal "$__fatals merge/minarea fatals — NO streamOut" }
foreach {__ni1 __ns1 __nw1} [__counts] {}
puts "C6E2 COUNTS post-patch: insts=$__ni1 sWires=$__ns1 wires=$__nw1 (post-eco $__nie/$__nse/$__nwe)"
...
streamOut \
	$OUTPUT_DIR/MCU_castalia.c6e2.gds2 \
	-libName WorkLib \
...
puts "C6E2: COMPLETE (trial GDS out/MCU_castalia.c6e2.gds2, no saveDesign)"
exit
